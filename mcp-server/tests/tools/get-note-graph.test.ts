...
describe('get_note_graph tool', () => {
  ...
  test.each([
    ...
    async (tokenLimit, expectedError) => {
      const result = await findTool('get_note_graph').handle(
        createMockContext(),
        { noteId: 1, tokenLimit }
      )
  ...
  test('returns graph JSON for a valid tokenLimit', async () => {
    const graph: GetGraphResponse = {
      focusNote: {
        title: 'Example',
        depth: 0,
        outgoingLinks: [],
    ...
    expect(result.content[0].text).toBe(JSON.stringify(graph))
  })
})
