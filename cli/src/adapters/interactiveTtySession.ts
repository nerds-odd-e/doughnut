...
export function runInteractiveTtySession(stdin: TTYInput, deps: TTYDeps): void {
  ...
  function handleShellRendered(session: ShellSessionState): void {
    // Interactive readiness contract:
    // - fetch-wait omits the ready control sequence (typing is blocked)
    // - confirm/list selection surfaces emit the ready sequence directly
  ...
  function InteractiveTtyInkApp(): React.ReactElement {
      ...
      switch (listDispatch.result) {
        ...
        case 'submit-with-line': {
          ...
          const inputForHistory =
            latest.commandInput.lineDraft || effectiveInput
          patch((s) => ({
            ...s,
            commandInput: clearLiveCommandLine(s.commandInput),
            numberedChoiceHighlightIndex: 0,
    ...
    return React.createElement(ShellSessionRoot, {
      session,
      deps,
      handlers,
    })
  ...
  shellInstance = render(React.createElement(InteractiveTtyInkApp), {
