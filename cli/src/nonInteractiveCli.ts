...
async function completeNotebookClone(notebookArgs: string[]): Promise<void> {
  ...
  console.log(
    `Cloned notebook ${notebookId} into ${destination}. Open and edit the files there with any ordinary local Git tool (Obsidian, an IDE, plain git). Publishing currently accepts one new commit directly on the accepted main containing either one or more added Markdown notes with optional edits, a new folder README alone or together with ordinary notes in that folder, one or more edited existing ordinary Markdown notes at unchanged paths, one isolated equal-content Markdown note remove/add pair that may change folder and/or filename, one isolated Markdown note deletion that leaves existing links authored, or one complete same-name subtree whose source has its own accepted README, every active descendant is represented, the destination parent already exists in accepted history, and bytes, modes, and relative paths stay unchanged; overwriting an existing note, and relocating or renaming together with a content edit in the same commit, are not supported yet. To preserve note identity, commit and publish the unchanged relocation or rename, wait for it to be accepted, then edit and separately commit and publish the content change. Authored referring links are not rewritten by a relocation or rename, so links to the old path may no longer resolve. Do not delete and recreate the note. A new folder is represented by its README.md and may include ordinary notes in the same commit. Run "donut notebook pull ${destination}" to receive newer accepted history. ${notebookPullNextSteps(destination)}`
  )
}
