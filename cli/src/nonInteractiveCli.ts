...
async function completeNotebookClone(notebookArgs: string[]): Promise<void> {
  ...
  console.log(
    `Cloned notebook ${notebookId} into ${destination}. Open and edit the files there with any ordinary local Git tool (Obsidian, an IDE, plain git). Publishing currently accepts one new commit directly on the accepted main containing either one or more added Markdown notes with optional edits, or a single edited Markdown note, or one isolated Markdown note deletion that leaves existing links authored; mixed deletion and same-path recreation are unsupported. Use the notebook root or existing folders represented in accepted history. Run "donut notebook pull ${destination}" to receive newer accepted history; accepted history may not include all current web content.`
  )
}
