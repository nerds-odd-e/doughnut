...
export function rebasedReport(
  ...
  return `Rebased onto the accepted history. Local head: ${localHead}. Accepted head: ${acceptedHead}. Inspect the result, then run "donut notebook publish ${directory}".`
}

export function alreadyBasedReport(
  directory: string,
  localHead: string,
  acceptedHead: string
): string {
