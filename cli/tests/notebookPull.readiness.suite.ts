...
export function describeNotebookPullReadiness(): void {
  describe('notebook pull (CLI routing and local readiness)', () => {
    const ctx = installNotebookCliRunFixture('donut-cli-pull-readiness-test-')
    ...
    test('missing directory reports pull usage without making a request', async () => {
      await expect(run(['notebook', 'pull'])).rejects.toThrow(
        ProcessExitForTest
      )

      expect(ctx.getErrorSpy()).toHaveBeenCalledWith(
        'donut: usage: donut notebook pull <directory>\nReceives accepted notebook history onto a clean local main. When one unpublished commit edits one existing note at an unchanged path and accepted history advanced through ordinary-note content changes at unchanged paths, through one added ordinary note at the root or an already represented folder, or through that addition followed by one content save of the same note, pull rebases that unpublished commit. Git auto-merges overlapping same-note content when it can; otherwise it pauses with a native conflict so you can edit, stage, and run git rebase --continue, or git rebase --abort. Pull does not publish. Inspect the result, then run "donut notebook publish <directory>" if unpublished work remains. Accepted history may not include all current web content.'
      )
