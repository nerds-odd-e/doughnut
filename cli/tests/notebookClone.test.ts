  ...
  test('clone downloads the accepted bundle and checks it out as a clean single-commit main branch', async () => {
    ...
    expect(ctx.getLogSpy()).toHaveBeenCalledWith(
      expect.stringContaining(
        'one new commit directly on the accepted main containing either one or more added Markdown notes with optional edits, a single edited Markdown note, one isolated equal-content Markdown note remove/add pair that may change folder and/or filename, or one isolated Markdown note deletion that leaves existing links authored; creating a new or unrepresented folder, overwriting an existing note, moving a folder or README, and relocating or renaming together with a content edit in the same commit, are not supported yet. To preserve note identity, commit and publish the unchanged relocation or rename, wait for it to be accepted, then edit and separately commit and publish the content change. Authored referring links are not rewritten by a relocation or rename, so links to the old path may no longer resolve. Do not delete and recreate the note. Use the notebook root or existing folders represented in accepted history.'
      )
    )
    expect(ctx.getLogSpy()).toHaveBeenCalledWith(
      expect.stringContaining(
        `Run "donut notebook pull ${destinationPath}" to receive newer accepted history. When one unpublished commit edits one existing note at an unchanged path and accepted history advanced through ordinary-note content changes at unchanged paths, pull rebases that unpublished commit. Git auto-merges overlapping same-note content when it can; otherwise it pauses with a native conflict so you can edit, stage, and run git rebase --continue, or git rebase --abort. Pull does not publish. Inspect the result, then run "donut notebook publish ${destinationPath}" if unpublished work remains. Accepted history may not include all current web content.`
      )
    )
  })
