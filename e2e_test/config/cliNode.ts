/**
 * Node-only helpers for running / bundling the Doughnut CLI in E2E (no Cypress).
 */
...
export const CLI_BUNDLE_PATH = 'cli/dist/doughnut-cli.bundle.mjs'

export const CLI_SPAWN_TIMEOUT_MS = 25_000
...
export function bundleAndCopyCliToBuildOutput(
  repoRoot: string,
...
export function getCliRunConfig(repoRoot: string): {
  command: string
  baseArgs: string[]
} {
  if (process.env.CI === '1') {
...
export async function spawnCliFromRepo(opts: {
  repoRoot: string
  args?: string[]
  stdin?: string
  env?: NodeJS.ProcessEnv
