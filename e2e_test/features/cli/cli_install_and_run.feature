...
Feature: CLI install and run
  As a learner, I want to install the Donut CLI from the server, check its version, use it interactively, and update it.
  ...
  Scenario: Installed CLI updates to a newer version from the server
    ...
    When I run the installed donut update command with BASE_URL from localhost
