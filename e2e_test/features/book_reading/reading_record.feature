Feature: Reading record
  ...
  Rule: From section 1, scroll within page past the next book block bbox
    ...
    Background:
      Given I choose the book block "1. Refactoring: Protecting Intention in Working Software"
      And I scroll the PDF book reader down within the same page to move viewport past the next book block bbox

    Scenario: Auto-read a heading-only book block when entering its successor
    ...
    Scenario: Current block differs from selected shows navigation affordance
      Then I should see the current block navigation bar showing "2.1 Easier to Change—and Harder to Misuse"
      When I start reading from the current block
    ...
    Scenario: Back to selected scrolls back to the selected block
      Then I should see the current block navigation bar showing "2.1 Easier to Change—and Harder to Misuse"
      When I go back to the selected book block
  ...
  Scenario: Mark a book block as read
  ...
  Scenario: Mark the last book block as read
