...
When('I download notebook {string}', (notebookTitle: string) => {
  cy.get(`[data-test="download-notebook-${notebookTitle}"]`).click()
})

When(
  'I download notebook {string} from the bazaar',
  (notebookTitle: string) => {
    cy.get(`[data-test="download-bazaar-notebook-${notebookTitle}"]`).click()
  }
)
...
Then(
  'the downloaded file should contain all notes from {string}',
  (notebookTitle: string) => {
...
Then(
  'I should see a download button for notebook {string}',
  (notebookTitle: string) => {
    cy.get(`[data-test="download-notebook-${notebookTitle}"]`).should(
      'be.visible'
    )
  }
)
...
Then(
  'I should not see a download button for notebook {string}',
  (notebookTitle: string) => {
    cy.get(`[data-test="download-notebook-${notebookTitle}"]`).should(
      'not.exist'
    )
