...
const bookReadingPage = () => {
  ...
  return {
    ...
    /**
     * Press and hold the third content block in the reading stream (eq(2)).
     * The first item (eq(0)) is the heading-text content block and cannot be split at
     * (API returns 400 for splitAt==0). The second body paragraph (eq(2), the third stream
     * item overall) gives title "A more practical view is this:..." which the E2E asserts.
     * Uses a native PointerEvent dispatched on the element to trigger the 500ms hold timer
     * in BookReadingContentStreamPanel.
     */
    pressAndHoldThirdContentStreamBlock() {
      pageIsNotLoading()
      cy.get(
