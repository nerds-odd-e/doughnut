# Plan: Read a block of a book (Story 2)

**User story:** [`ongoing/book-reading-user-stories.md`](book-reading-user-stories.md) — *Story: Read a block of a book*.

**Architecture (boundaries, storage, pdf.js, drawer, two-way sync):** [`ongoing/doughnut-book-reading-architecture-roadmap.md`](doughnut-book-reading-architecture-roadmap.md) — *Story 2 — Read a block (direction)*.
...
**User outcome:** The **book layout** (`BookBlock` tree) from **`GET .../book`** appears in a **drawer sidebar**; the **PDF viewer stays in the main** region. **E2E:** layout tree visible alongside rendered PDF (drawer open by default or clearly openable—pick one consistent default and assert it).
...
## Phase 5 — Click a book block → PDF jumps to the right place — **done**

**User outcome:** **Selecting a book block** in the **book layout** navigates **pdf.js** to the block’s **start (or agreed) anchor** (page/region per **`pdf.mineru_outline_v1`**). **E2E:** click known **book block** → observable scroll/page change or focus.

**Shipped:** `mineruOutlineV1PageIndex` (`parseMineruOutlineV1StartAnchor`, MinerU **0–1000** bbox → pdf.js XYZ + top padding), `PdfBookViewer` `scrollToMineruOutlineV1Target`, `BookReadingPage` **book block** activation and chrome/safe-area handling; Cypress scenarios *Book block jumps the PDF to the anchored page* and *Same-page scroll moves the current block; the current selection stays the explicit choice* in [`e2e_test/features/book_reading/book_browsing.feature`](../e2e_test/features/book_reading/book_browsing.feature). **Contract:** see **Important learnings** in [`ongoing/book-reading-e2e-refactoring-fixture-visible-ocr-plan.md`](book-reading-e2e-refactoring-fixture-visible-ocr-plan.md).
...
## Phase 6 — Scroll / navigate the PDF → active block highlight in the layout — **done**

**User outcome:** As the user **scrolls or changes page** in the PDF, the **layout** shows which **BookBlock** is **current** (highlight / active row). **E2E:** scroll to a region tied to a different block → highlight moves (or equivalent DOM assertion).

**Shipped:** `PdfBookViewer` emits a viewport descriptor (`viewportAnchorPage`: page index, **within-page Y as MinerU-normalized 0–1000** midpoint, `pagesCount`). `currentBlockAnchorIdFromAnchorPage` in `currentBlockAnchorFromAnchorPage.ts` compares that to each anchor’s **`y0`** in the same 0–1000 system and picks the matching flat **book block** row; debounced updates via `debounceCurrentBlockAnchorId.ts` / `createCurrentBlockAnchorDebouncer`. **Book blocks** expose `data-current-block` and separate **current selection** (Phase 5) vs **current block** styling and ARIA. `BookReadingPage` scrolls the aside list so the **current block** stays visible when the **book layout** is open; visually hidden `aria-live="polite"` for title changes (`currentBlockLiveAnnouncement.ts`). Cypress scenarios *Scrolling the PDF updates the current block*, *Short viewport scrolls book layout aside so the current block stays visible*, and *Same-page scroll moves the current block; the current selection stays the explicit choice* in [`e2e_test/features/book_reading/book_browsing.feature`](../e2e_test/features/book_reading/book_browsing.feature). Unit coverage: `currentBlockAnchorFromAnchorPage.spec.ts`, `currentBlockLiveAnnouncement.spec.ts`; drawer-closed state: [`BookReadingPage.spec.ts`](../frontend/tests/pages/BookReadingPage.spec.ts).

---

## Phase 7 — Show / hide the layout drawer

**User outcome:** User can **toggle** the drawer for **more reading space** without losing session state (current block / scroll should remain sensible when reopening). **E2E:** toggle control works; PDF still usable.
...
**User outcome:** Focus a **book block** control and activate with **Enter** / **Space** (platform-appropriate) → same PDF navigation and **current selection** state as pointer click; touch must not regress.

**E2E:** One scenario path: keyboard activation reaches the same postcondition as **book block** click → page/scroll and **current selection** contract as in Phase 5.
...
**User outcome:** If the **book layout** gains **hierarchical** **book blocks** with expand/collapse (e.g. chevron), **go-to** must not fire when toggling expansion (chevron vs row body or equivalent pattern). **Skip** this phase while the **book layout** stays a **flat** list with no expand control.
...
- **Reading record** and **next block** stories build on **BookBlock** + **current block** from Phases 5–6.
