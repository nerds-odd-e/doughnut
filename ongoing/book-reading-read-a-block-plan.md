...
**Shipped:** `PdfBookViewer` emits a viewport descriptor (`viewportAnchorPage`: page index, **within-page Y as MinerU-normalized 0–1000** midpoint, `pagesCount`). `currentBlockAnchorIdFromAnchorPage` in `currentBlockAnchorFromAnchorPage.ts` compares that to each anchor’s **`y0`** in the same 0–1000 system and picks the matching flat **book block** row; debounced updates via `debounceCurrentBlockAnchorId.ts` / `createCurrentBlockAnchorDebouncer`. **Book blocks** expose `data-current-block` and separate **current selection** (Phase 5) vs **current block** styling and ARIA. `BookReadingPage` scrolls the aside list so the **current block** stays visible when the **book layout** is open; visually hidden `aria-live="polite"` for title changes (`currentBlockLiveAnnouncement.ts`). Cypress scenarios *Scrolling the PDF updates the current block; short viewport keeps aside scrolled to it* and *Same-page scroll moves the current block; the current selection stays the explicit choice* in [`e2e_test/features/book_reading/book_browsing.feature`](../e2e_test/features/book_reading/book_browsing.feature). Unit coverage: `currentBlockAnchorFromAnchorPage.spec.ts`, `currentBlockLiveAnnouncement.spec.ts`; drawer-closed state: [`BookReadingPage.spec.ts`](../frontend/tests/pages/BookReadingPage.spec.ts).
