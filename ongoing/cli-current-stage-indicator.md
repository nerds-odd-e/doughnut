...
- Remaining wrapped Current prompt lines (if any), grey foreground in `buildLiveRegionLines`
- **input box**
- **Current guidance**
...
_None._ **Access token list / pick** shipped: `AccessTokenPickerCommandConfig.stageIndicator` + `greyCurrentStageIndicatorLabel` in `renderer.ts`; `getDisplayContent` / `currentStageIndicatorLinesForLiveRegion` in `ttyAdapter.ts`; labels only in `TOKEN_LIST_COMMANDS` (`interactive.ts`).
