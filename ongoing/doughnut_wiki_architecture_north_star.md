...
5. **JPA for pointer associations** — In application and test code, map cached index pointers (and similar nullable FKs to `Note`) with JPA entity associations (for example `@ManyToOne` on `Notebook` / `Folder` to `Note`) and update them through the entity model and normal persist/merge/flush paths. Prefer that over ad-hoc JDBC or string SQL that updates `index_note_id` columns directly. **Flyway** remains the right place for schema DDL and one-time backfills.
