...
**User outcome:** After “Yes, I remember?” → **Spell:** prompt; typed `sedition` → continue session (companion tracker + load-more **n** → **Recalled 2 notes** when two trackers are due).
...
- **Done:** `spelling-session` variant from `loadNextRecallCardIfAny` when `mt.spelling`; `SpellingRecallStage` owns memory prompt + spell phases (`fetchSpellingRecallPrompt`, `submitSpellingAnswer`, `Spell:` + stem); `markMemoryTrackerRecalled` shared with just-review; due queue tie-break `IFNULL(spelling,0) DESC`; no spelling fields on `RecallJustReviewPayload`; no `onAssistantLine` on interactive shell.
