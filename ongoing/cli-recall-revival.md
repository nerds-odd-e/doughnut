...
- **Done:** `spelling-session` when `mt.spelling`; `SpellingRecallStage` fetches spelling prompt on mount (spell first, no prior just-review on that tracker); `CliTranscriptAppendContext` appends **Correct!** when advancing to the next due tracker; due queue tie-break `IFNULL(spelling,0) DESC`; slim spelling payload (`memoryTrackerId` + notebook); just-review remains MCQ fallback only.
