...
### Phase 7.7 — Edge cases (Vitest scope) — **complete**

- **MCQ:** **n** on leave confirm → back to MCQ with stable highlight (↓ then Enter submits second choice → `answerQuiz` `choiceIndex: 1`) and stable typed buffer (`> z`); empty **Enter** on leave confirm leaves confirm up then **n** returns.
- **Just-review:** **n** on leave confirm → exactly one `Yes, I remember?` in frame; empty **Enter** on leave confirm then **n** returns.
- **Spelling:** empty **Enter** on leave confirm then **n** → `Spell:` + prior buffer preserved (extends **7.4** buffer test).
- Invalid committed line on leave confirm: empty Enter covered in recall flows; non-y/n still covered by **`YesNoStagePrompt`** unit tests.
...
### Phase 9.3 — Edge cases (scenario scope only) — **complete**

- **Contest when not allowed / API error:** Rejected contest shows backend `advice` (or fallback) in the recall session answered strip; contest failures map through `userVisibleSlashCommandError` / backend client messaging — Vitest in `recallMcqInteractive.test.tsx` and `contestAndRegenerateMcq.test.ts`.
- **Regenerate returns same stem:** Behavior defined by backend; CLI displays what it receives — no client-side test (no comparison logic).
