...
- **Done:** `spellingPhaseAfterReview` on just-review payload; `askAQuestion` → `RecallSpellingStage` (`Spell:` in guidance, stem via `renderMarkdownToTerminal`, `submitSpellingAnswer`); due queue tie-break so spelling trackers sort before non-spelling when `next_recall_at` ties (`MemoryTrackerRepository`); optional `onAssistantLine` on interactive stages for `Correct!` before loading the next due tracker; spelling scenario completes companion just-review + load-more **n** and expects `Recalled 2 notes`.
