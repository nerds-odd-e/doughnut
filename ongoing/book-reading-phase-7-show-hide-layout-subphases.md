...
**Production change:** None if behavior already matches; otherwise minimal fix.
...
**Production:** Add `id` on aside only if `aria-controls` is used; otherwise minimal `aria-expanded` on the button.
...
1. Prefer **one** new failing test (or one focused `it` block) for the slice, then minimal production change.
