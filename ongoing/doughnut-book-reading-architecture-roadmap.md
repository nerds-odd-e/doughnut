...
**Implemented so far (Story 2):** Phases **1–4** of that plan are shipped: multipart attach, **`GET …/book/file`**, **pdf.js** full scrollable viewer (`PdfBookViewer` using `PDFViewer` from `pdfjs-dist/web/pdf_viewer.mjs`), and the **outline** from **`GET …/book`** in a **left** responsive drawer/panel (PDF in **`main`**; **768px** breakpoint: open by default on large, overlay + backdrop on small; **Outline** control in **GlobalBar**). **Outline ↔ PDF sync** is not implemented yet. The book-reading E2E asserts that page-1 content is present by **OCR on the first page canvas** in Cypress test infrastructure (Tesseract.js, committed language data under `e2e_test/tesseract/`) — **not** via a product DOM text layer on top of the canvas.
