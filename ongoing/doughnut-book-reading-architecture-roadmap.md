...
A region: **navigation geometry** comes from **`contentLocators`** on **`GET …/book`**—an ordered list of **`ContentLocator`** with format-specific variants (PDF = `{pageIndex, bbox}`; EPUB = `{href, fragment?}`). **`contentLocators[0]`** is the block start (derived from the structural heading’s imported raw data); further entries cover qualifying direct-content regions. Derived rule: **`hasDirectContent := contentLocators.length > 1`** works format-agnostically. Primary unit for **navigation**, **hierarchical decomposition**, and **progress**. **`structuralTitle`** is the human-readable label for that **book block** in the **book layout** (e.g. `Chapter 3`, `2.4.1`). A breadcrumb-style path can be **derived** by walking parent blocks; we do not use a separate persisted “structural address” field.
...
- The heading item that introduces a `BookBlock` is represented on **`BookBlock`** (title + **`contentLocators[0]`** from its raw data); it is **not** duplicated as a `BookContentBlock` and is **not** counted as direct body content by default.
...
1. Every `BookBlock` has **`contentLocators`** with a well-defined first entry for block start (PDF: MinerU-normalized page + bbox; EPUB: spine href + optional `#fragment`); further entries optional for direct content.
...
- **Direct content** remains a **product concept**, not a materialized “gap blob.” **Disposition** persistence is **`ReadingRecord`** rows + **`PUT`/`GET` reading-record APIs** per [`ongoing/book-reading-reading-record-plan.md`](book-reading-reading-record-plan.md); persisted `BookContentBlock` rows supply the import evidence for **`hasDirectContent`**. On the wire, each block on **`GET …/book`** exposes **`contentLocators`**: an ordered list of **`ContentLocator`** (OpenAPI discriminated union `PdfLocator_Full` + `EpubLocator_Full` — PDF = page index + normalized bbox; EPUB = spine `href` + optional `#fragment`). The **first** entry is the block start; further entries come from qualifying direct-content imported blocks. The reader uses the **last** entry as the bottom of direct content when **`contentLocators.length > 1`**. When the predecessor has no direct content, the reader **auto-marks** it read on successor entry (shared across PDF and EPUB); otherwise the **Reading Control Panel** is the primary explicit control — it appears when the bottom of the selected block’s last direct-content locator is visible above the panel’s obstruction zone (geometry-gated), **anchors** just under that bottom and tracks on scroll when that geometry holds, otherwise stays **bottom-docked** in the main pane, and remains available until the successor block becomes the viewport-derived current block; see [`ongoing/book-reading-ux-ui-roadmap.md`](book-reading-ux-ui-roadmap.md).

---

## Story 1 (shipped)

**Book** metadata plus **BookBlock** tree on a **Notebook**: **`POST /api/notebooks/{notebook}/attach-book`** (JSON **book layout** / `BookBlock` tree only) and **`GET /api/notebooks/{notebook}/book`**, at most one book per notebook. As shipped, **`sourceFileRef` is not used** and there is **no server-side PDF storage**; the PDF stayed on the client. Layout import supplies structural nodes with **`contentBlocks`**; the server derives **`contentLocators`** (PDF variant: MinerU page index + normalized bbox) for each **`BookBlock`**.
...
**EPUB (direction, Apr 2026):** Render EPUB in the **main pane** with **Readium Web** ([`readium/ts-toolkit`](https://github.com/readium/ts-toolkit): **`@readium/navigator`**, **`@readium/shared`**, and related loading/parsing packages as required). **Reading position** and future **citation** locators live on the shared `ContentLocator` (EPUB variant: spine `href` + optional `#fragment`) with room to grow toward Readium’s **`Locator`** (CFI, progression) per §2.3 in [`ongoing/book-reading-epub-research-quest.md`](book-reading-epub-research-quest.md). Scroll vs paginated mode, **mobile Safari**, and **bundle** are **spike-gated** before ship.

**EPUB reading-record seam (shipped):** The EPUB reader (`BookReadingEpubView`) goes through the **same** `useNotebookBookReadingRecords` composable and the **same** `BookReadingBookLayout.dispositionForBlock` prop as the PDF reader — there is no parallel records state or layout contract for EPUB. The `ReadingControlPanel` component is reused as-is, anchored below the last direct-content locator via the shared `BookReaderViewerRef` contract. The "mark and advance to the next block" rule is shared via `frontend/src/lib/book-reading/nextBookBlockAfter.ts`; the only format-specific piece is applying the advance (PDF scrolls to the bbox; EPUB calls `displayLocator(block.contentLocators[0])`).
...
**Implemented so far (Story 2):** Phases **1–6** and **11–13** of that plan are shipped: multipart attach, **`GET …/book/file`**, **pdf.js** full scrollable viewer (`PdfBookViewer` using the **modern** pdf.js stack — see **PDF.js build** below), the **book layout** from **`GET …/book`** in a **left** responsive drawer/panel (PDF in **`main`**; **768px** breakpoint: open by default on large, overlay + backdrop on small; **Book layout** control in **GlobalBar**), **layout → PDF** navigation from each block’s **`allBboxes`** (structured **`PageBbox`**: page index, optional bbox, chrome/safe-area offset, bad-target no-op), **PDF ↔ book layout** sync (viewport drives **current block** highlight on a **book block**, debounced, with accessible live region for title changes), responsive default PDF scale (full-width on narrow viewports, comfortable max-width cap on wide viewports based on first-page intrinsic geometry, with manual zoom preserved across resize), **GlobalBar** **`PdfControl`** zoom buttons + page indicator, and **PDF-only gesture zoom** (ctrl/meta + wheel and two-finger pinch on the viewer scroll container, shared **`pdfViewer.currentScale`**, `preventDefault` to avoid browser zoom). The book-reading E2E uses **OCR on rendered canvases** (Tesseract.js, committed language data under `e2e_test/tesseract/`) for page markers — **not** a product DOM text layer on top of the canvas.

**PDF.js build (current):** The app uses the **non-legacy** pdf.js entrypoints from **`pdfjs-dist`**: `build/pdf.mjs` (e.g. `getDocument`), `web/pdf_viewer.mjs` + `web/pdf_viewer.css` (`PdfBookViewer`), and the stock **`build/pdf.worker.mjs`** loaded via a small Vite worker entry. **`frontend/src/lib/pdfjsWorker.ts`** sets `GlobalWorkerOptions.workerSrc` from **`pdfWorkerEntry.ts?worker&url`** (Vite’s **worker** pipeline). Do **not** use plain **`?url`** for this entry: Vite can inline it as a **`data:`** script in some environments (e.g. GitHub Actions + Cypress Electron), which breaks **`import()`** inside the worker (`Failed to fetch dynamically imported module`, often with a bogus MIME like `video/mp2t`). **`worker.format`** is **`es`** in **`frontend/vite.config.ts`** so the bundle matches pdf.js’s **`type: 'module'`** workers. That entry imports polyfills, then the real worker. **`pdfjs-dist` stays in `optimizeDeps.exclude`** so Vite does not pre-bundle pdf.js separately from the worker (avoids broken or “fake” worker behavior). **Main thread and worker must stay on the same pdf.js version** (same `pdfjs-dist` release); do not point `workerSrc` at a worker from a different build line (that mismatch surfaces as internal handler errors, e.g. `getOrInsertComputed`).

**Cypress Electron vs modern pdf.js (why there are still “legacy-era” workarounds):** We previously used the **legacy** pdf.js bundle because it polyfilled **`Uint8Array.prototype.toHex`**, which pdf.js 5.6 uses in the worker for PDF fingerprints; **Cypress’s default `cypress run` browser (Electron)** did not expose `toHex` in the worker realm, which broke load (`hashOriginal.toHex is not a function`). After moving to the modern stack, the same Electron runtime also lacked **`Map.prototype.getOrInsertComputed`**, which pdf.js 5.6 uses on both main thread and worker (`this[#methodPromises].getOrInsertComputed is not a function`). **Mitigation (current):** `frontend/src/lib/pdfJsMapGetOrInsertComputedPolyfill.ts` runs on main (via `pdfjsWorker.ts` before `build/pdf.mjs`) and in the worker entry; `frontend/src/lib/pdfUint8ArrayToHexPolyfill.ts` runs in the worker entry before `build/pdf.worker.mjs`. **Reminder — remove when safe:** When **Cypress ships an embedded Electron / Chromium** where both APIs exist in the **same contexts pdf.js uses** (verify with default Electron E2E, e.g. `book_reading/book_browsing.feature`), **delete the polyfill modules**, collapse **`pdfWorkerEntry.ts`** to a direct re-export of **`pdf.worker.mjs`** (or set `workerSrc` to **`pdf.worker.mjs?worker&url`** only), and drop the extra `import "./pdfJsMapGetOrInsertComputedPolyfill"` from **`pdfjsWorker.ts`**. Keep **`?worker&url`** (or another Vite worker URL pattern) if plain **`?url`** still inlines badly in CI. Re-check **`optimizeDeps.exclude`** for `pdfjs-dist` after any change; keep excluding it unless a future Vite/pdf.js combo proves pre-bundling is safe.
