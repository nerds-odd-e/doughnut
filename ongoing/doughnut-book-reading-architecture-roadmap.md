...
> **Direction (EPUB Phase 8 in flight):** `allBboxes` / `PageBbox` / `epubStartHref` are being generalized into `contentLocators: List<ContentLocator>` with PDF and EPUB variants (PDF = `{pageIndex, bbox}`; EPUB = `{href, fragment?}`). Semantics carry over unchanged — index 0 is the block start, index 1+ are direct-content entries — so `hasDirectContent := contentLocators.length > 1` works for both formats. See [`book-reading-epub-phase-8-sub-phases.md`](book-reading-epub-phase-8-sub-phases.md). Once that lands this subsection will be rewritten against the generalized names.
