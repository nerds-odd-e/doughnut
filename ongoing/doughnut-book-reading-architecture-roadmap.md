...
- **`GET …/book` layout wire:** Each block includes persisted **`depth`** (≥ 0); OpenAPI documents **`blocks`** as depth-first preorder matching ascending **`layout_sequence`**. The **browser reader** and **CLI `/attach` preview** consume **array order** and **`depth`** directly ([`ongoing/book-reading-book-block-flat-outline-plan.md`](book-reading-book-block-flat-outline-plan.md) Phase 4). Derived **`parentBlockId`** and **`siblingOrder`** remain on the wire until interim dual-field serialization is removed (**After all phases** in that plan).
