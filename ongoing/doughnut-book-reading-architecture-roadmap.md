...
- **Direct content** remains a **product concept**, not a materialized “gap blob.” **Disposition** persistence is **`ReadingRecord`** rows + **`PUT`/`GET` reading-record APIs** per [`ongoing/book-reading-reading-record-plan.md`](book-reading-reading-record-plan.md); persisted `BookContentBlock` rows supply the import evidence for **`hasDirectContent`** and **`lastDirectContentBbox`** (last qualifying content block’s page index + MinerU bbox, exposed on `BookBlockFull`). When the predecessor has no direct content, the reader **auto-marks** it read on successor entry (Phase 3, shipped); otherwise the **Reading Control Panel** is the primary explicit control — it appears when the bottom of the selected block’s last direct-content bbox is visible above the panel’s obstruction zone (geometry-gated; see [`ongoing/book-reading-ux-ui-roadmap.md`](book-reading-ux-ui-roadmap.md)).
