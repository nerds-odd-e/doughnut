...
**E2E fixture / MinerU mock migration (`refactoring.pdf`, committed JSON, real MinerU refresh):** Sub-phased delivery in [`ongoing/book-reading-e2e-refactoring-fixture-visible-ocr-plan.md`](book-reading-e2e-refactoring-fixture-visible-ocr-plan.md) — **Phase 1** = **sp-1.1** (PDF-only swap + fix OCR-driven failures) then **sp-1.2** (same mock content in JSON); **Phase 2** = **sp-2.1** (script, capture to a non-canonical JSON name) then **sp-2.2** (promote to `mineru_output_for_refactoring.json` + fix E2E). **Phase 3** (beginning-of-book OCR using substring **`Code Refactoring`** on the first page canvas) is **shipped**; optional scrollport ∩ canvas crop for that step remains open. **Phase 4** (outline-jump scenario: Cypress element screenshot of the PDF viewer + OCR; MinerU XYZ scroll target offset above bbox top) is **shipped**. **Phases 5–7** cover remaining visible-viewport OCR for page markers / scroll scenarios and positioning fixes driven by failing tests.
