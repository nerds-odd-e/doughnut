...
**E2E fixture / MinerU mock migration (`refactoring.pdf`, committed JSON, real MinerU refresh):** Sub-phased delivery in [`ongoing/book-reading-e2e-refactoring-fixture-visible-ocr-plan.md`](book-reading-e2e-refactoring-fixture-visible-ocr-plan.md) — **Phase 1** = **sp-1.1** (PDF-only swap + fix OCR-driven failures) then **sp-1.2** (same mock content in JSON); **Phase 2** = **sp-2.1** (script, capture to a non-canonical JSON name) then **sp-2.2** (promote to `mineru_output_for_refactoring.json` + fix E2E). Further phases there cover visible-viewport OCR and positioning fixes driven by failing tests.
