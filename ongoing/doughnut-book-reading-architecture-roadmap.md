...
**Implemented so far (Story 2):** Phases **1–5** of that plan are shipped: multipart attach, **`GET …/book/file`**, **pdf.js** full scrollable viewer (`PdfBookViewer` using `PDFViewer` from `pdfjs-dist/web/pdf_viewer.mjs`), the **outline** from **`GET …/book`** in a **left** responsive drawer/panel (PDF in **`main`**; **768px** breakpoint: open by default on large, overlay + backdrop on small; **Outline** control in **GlobalBar**), and **layout → PDF** navigation from **`pdf.mineru_outline_v1`** anchors (page index, optional bbox, chrome/safe-area offset, bad-anchor no-op). **PDF → outline** (viewport drives **current** range highlight) is **Phase 6** — not implemented yet. The book-reading E2E uses **OCR on rendered canvases** (Tesseract.js, committed language data under `e2e_test/tesseract/`) for page markers — **not** a product DOM text layer on top of the canvas.
