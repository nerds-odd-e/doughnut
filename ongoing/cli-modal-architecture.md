...
**Phase 5 (done):** MCQ and token lists use Ink **`RecallMcqChoicesLivePanel`** / **`AccessTokenPickerLivePanel`** in **`liveSelectionGuidanceInk.tsx`** (`useInput` + **`selectListInteraction`**); readline **`keypress`** handles **Esc** on those lists only (bridge). **Phase 3 (done):** stop-confirm + session y/n use Ink **`ConfirmLivePanel`**. **Phase 6 (done):** TTY stop-confirm + session y/n use **`@inkjs/ui` `ConfirmInput`** (disabled for themed **`y/N`** label only) + **`StatusMessage`** for invalid keys; one Ink **`useInput`** owns keys (see phase 6). **Phase 7:** delete leftover duplicate **`keypress`** paths after phases 2–6 (including the list Esc bridge if a single stdin owner can be proven).
...
| Topic | Choice |
|-------|--------|
| Typed `yes` / `no` + Enter | **Not supported** on TTY — **y** / **n** only. Piped **`processInput`** unchanged (**`parseRecallSessionYesNoSubmit`**). |
...
**Shipped:** **`@inkjs/ui`** dependency. **`cli/src/ui/ConfirmLivePanel.tsx`:** one **`useInput`** + **`eachLogicalInkStdinChunk`**; **`dispatched`** guard per stdin callback for **`y`+Enter** coalescing; **`ConfirmInput`** **`isDisabled`** for **`y/N`** styling only. Removed **`ConfirmDisplay`**, **`dispatchRecallSessionConfirmKey`**; **`SessionYesNoLineDispatchResult`** narrowed to three outcomes. **`ttyAdapter`:** no **`emptySubmit`** prop.
...
| Confirm / y/n | **`ConfirmLivePanel`** + **`@inkjs/ui` `ConfirmInput`** (disabled label) + **`StatusMessage`** (phase 6) | No duplicate |
