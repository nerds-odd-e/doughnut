...
**Order (extended track):** **~~9~~ (done)** → **~~9.5~~ (done)** → **~~10~~ (done)** → **~~10.5~~ (done)** → **11 → 11.5 → 12 → 13 → 14** — **~~10.5~~** = command-line **input chrome**: bordered `Box` removed from **`CommandLineLivePanel`** (borderless prompt row + full-width paint lines); **11** = thin TTY + Ink root state (today’s command-line **`useInput`** + reverse-video caret, no bordered input box); **11.5** = **gate 8** command line → **`@inkjs/ui` `TextInput`** (and peel bespoke caret / **`hideCursor`** when owned); then **shrink `renderer.ts`**, **`patchConsole: true`**, **final residue audit**.
