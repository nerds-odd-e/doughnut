...
- **Cursor and command input** follow **Ink / `@inkjs/ui` convention** (e.g. **`TextInput`** or Ink’s caret behavior) — **no obligation** to keep today’s reverse-video caret, bordered box, or hidden-hardware-cursor pairing if something else fits Ink better (**phase 10.5** removes the bordered input box by default; **gate 8** / **phase 11.5** for caret/**`TextInput`** if not folded into **11**).
...
**Order (extended track):** **~~9~~ (done)** → **~~9.5~~ (done)** → **~~10~~ (done)** → **10.5 → 11 → 11.5 → 12 → 13 → 14** — **10.5** = command-line **input chrome**: drop bordered `Box` (prefer **no border**; optional **background-only** if that adds no extra abstraction); **11** = thin TTY + Ink root state (may keep today’s command-line **`useInput`** + reverse-video caret — **after 10.5**, no bordered input box); **11.5** = **gate 8** command line → **`@inkjs/ui` `TextInput`** (and peel bespoke caret / **`hideCursor`** when owned); then **shrink `renderer.ts`**, **`patchConsole: true`**, **final residue audit**.
...
### Phase 10.5 — Command-line input: no border (optional background-only)

**User outcome:** The main command-line input is **easier to maintain and test** — **no** bordered `Box` around the editable line (**gate 6**). Default is **no border**. If a subtle **background-only** treatment improves scanability **without** new helpers or branching, that is in scope; **do not** add complexity to “replace” the border.

**Goal:** Remove border-driven layout and snapshot/test coupling in **`CommandLineLivePanel`** (and any shared helper used only for that frame). **Out of scope:** **`ttyAdapter`** deletion (**11**), **`@inkjs/ui` `TextInput`** (**11.5** / gate 8), **`renderer.ts`** shrink (**12**). Keep existing **`useInput`** line editor and caret model unless a **minimal** adjustment is required for the new chrome.

**Verify (observable first):** **`pnpm cli:test`** — interactive / Ink paths that assert terminal output; targeted E2E (**`cli_interactive_mode`**, recall as needed). Prefer assertions on **what appears on the TTY** (prompt, typed text, guidance), not on internal **`drawBox`** structure unless the suite already standardizes on that surface.
