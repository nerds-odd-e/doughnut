...
**Phase 5 (done):** MCQ and token lists use Ink **`NumberedChoiceListLivePanel`** / **`TokenListLivePanel`** (`useInput` + shared list policy); readline **`keypress`** handles **Esc** on those lists only (bridge). **Phase 3 (done):** stop-confirm + session y/n use Ink **`ConfirmLivePanel`**. **Phase 6 (planned):** replace that with **`@inkjs/ui` `ConfirmInput`** under an **explicitly slimmer UX** (see phase 6). **Phase 7:** delete leftover duplicate **`keypress`** paths after phases 2–6 (including the list Esc bridge if a single stdin owner can be proven).
...
- **Shipped:** Hand-rolled Ink **`useInput`** in **`NumberedChoiceListLivePanel`** and **`TokenListLivePanel`** (plan fallback; no **`@inkjs/ui`** dependency). Policy stays in **`selectListInteraction.ts`** via **`dispatchSelectListKey`** + **`selectListKeyEventFromInk`**.
- **`ttyAdapter`:** list arrows / Enter / printable draft / backspace go through Ink; **`keypress`** returns early for those modes. **Esc** on MCQ (stop-confirm) and token list (abort) is handled on **`keypress`** as a small bridge so it is not lost vs Ink/readline ordering.
- **Focus:** **`LIST_SELECT_INK_FOCUS_ID`** on list panels; command line keeps Esc refocus **`useLayoutEffect`**; **`ConfirmLivePanel`** no longer uses the removed reflex flag.
...
| Lists (MCQ, tokens, selection) | Ink **`NumberedChoiceListLivePanel`** / **`TokenListLivePanel`** + **`selectListInteraction`**; Esc bridge on **`keypress`** | No duplicate list keys except Esc bridge |
