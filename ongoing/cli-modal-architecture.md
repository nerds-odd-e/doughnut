...
**Today (until phase 4 ships):** MCQ / token list still use readline `keypress`; **phase 5** removes leftover adapter/readline duplicates. **Phase 3 (done):** stop-confirm + session y/n use Ink `ConfirmLivePanel` (`useInput` + `dispatchRecallSessionConfirmKey`).
...
| Confirm / y/n | **`ConfirmLivePanel`** / **`useInput`** (phase 3) | No duplicate |
