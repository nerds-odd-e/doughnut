...
**Today (until phase 2 ships):** readline still owns typing and list keys; phases **2 → 4** move ownership into Ink; **phase 5** removes leftover adapter/readline duplicates.
...
**Order:** **2 → 3 → 4 → 5 → 6 (optional) → 7** (phase 1 done)
...
- **Complete replacement (this phase):** with shell active, **readline must not** still process the same keys for the main command line; **`ttyAdapter`** must not duplicate them.
