...
**Order (extended track):** **~~9~~ (done)** → **~~9.5~~ (done)** → **~~10~~ (done)** → **~~10.5~~ (done)** → **~~11~~ (done)** → **~~11.5~~ (done)** → **12 → 13 → 14** — **~~10.5~~** = command-line **input chrome**: bordered `Box` removed from **`CommandLineLivePanel`** (borderless prompt row + full-width paint lines); **11** = thin TTY + Ink root state (today’s command-line **`useInput`** + reverse-video caret, no bordered input box); **~~11.5~~** = **gate 8** caret aligned with **`@inkjs/ui` `TextInput`** (`chalk.inverse` + fetch-wait disabled **`TextInput`**); editable multiline line still one **`useInput`** (see phase **11.5** note); then **shrink `renderer.ts`**, **`patchConsole: true`**, **final residue audit**.
