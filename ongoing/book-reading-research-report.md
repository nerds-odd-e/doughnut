...
Doughnut can differentiate by making **structure-first reading** the primary UX: treat each book as a navigable knowledge tree/graph, allow BFS/DFS traversal, store multi-level summaries per node, and keep every extracted note “anchored” with stable **`BookAnchor`** pairs and a clear place in the **`BookRange`** tree (not just a page number). citeturn10search13turn9search0turn22search0turn12search4
...
flowchart TD
  ...
  B --> C[Build BookRange tree: structuralTitle + anchors]
...
For Doughnut, the strongest UX is to keep extraction *atomic and anchored*: each extracted **`Note`** ties to a **`Book`** (via notebook containment and optional **`SourceSpan`**), with evidence carried by **`BookAnchor`** pairs; the navigable hierarchy stays on **`BookRange`** (`structuralTitle` plus anchor-bounded regions). Rendering fallbacks (e.g., screenshot for scanned PDFs) can attach to span kind or media later without collapsing “section” and “citation” into one type. This matches the direction of “jump back to source” practices across tools. citeturn9search0turn10search13turn20search1
...
Doughnut can win by standardizing a robust “anchor format” (range tree position via `structuralTitle` / parent chain + precise coordinates in `BookAnchor` + fallback screenshot) that never breaks.
