...
1. **Doughnut UI:** Wiki-style `[[link]]` in note **content** are **clickable**. Treat the tree like **Wikipedia-style subpages**: within a notebook, the path from the notebook’s root notes down the hierarchy maps to link paths (top-level notes are the first segment; children extend the path).
   ...
   - **1.6** As a note taker, I want an **insert link** helper (e.g. button or command) to pick a target and insert a wiki link into **content**.
...
5. User can **upload** the optional root **`index`** note body (content) from local to Doughnut.

6. User can **push** local edits for notes that have ids in frontmatter (title, content, and related fields as defined by the sync contract).
...
1. **Doughnut UI:** ノート **content** 内の wiki 形式 `[[link]]` をクリック可能にする。**notebook** 内のツリーは **Wikipedia** の **subpage** に近い考え方で扱う：**notebook** のルートから下る階層が **link** のパスに対応し、最上位ノートがパスの先頭セグメント、子がパスを延ばす。
   ...
   - **1.6** ノート利用者として、**insert link** ヘルパー（例: **button** やコマンド）で対象を選び、**content** に wiki **link** を挿入したい。
...
5. **upload** でルートの **`index`** ノートの本文（**content**）を **local** → **Doughnut** に送れる。

6. **frontmatter** に **id** のあるノートについて、**local** の編集を **push** できる（**title**、**content**、および **sync contract** で定めた関連フィールド）。
