...
- **Target architecture:** `ongoing/doughnut_wiki_architecture_north_star.md`
- **Admin wiki reference batches (relationship backfill, etc.):** `ongoing/admin-wiki-reference-migration-redo-plan.md` and `ongoing/admin-wiki-reference-migration-status.md`
- **Folder-first listing and structural follow-ons:** `ongoing/doughnut_wiki_migration_plan-phase-6-sub-phases.md` — **Phase 6 complete** (see Status table there). Next: slug/path retirement (**boundary** after Phase 6), **Phase 7** — remove legacy note parent (`ongoing/doughnut_wiki_architecture_north_star.md`).
...
Phases **1–6** above remain the historical description of shipped work—including **persisted **`folder.slug`** / **`note.slug`**, **`unique(notebook_id, slug)`**, and slug/path routing introduced in Phase 2. That wording is deliberately unchanged.

Immediately **before** removing **`Note.parent`** (Phase **7** below), the product finishes **eliminating persisted slug/path identity**: drop **`note.slug`** and **`folder.slug`** (schema and backups), slug-keyed uniqueness constraints, slug-path lookups and resolving routes written for Phase 2, and any UX that treated path strings as the canonical note address—per **`ongoing/doughnut_wiki_architecture_north_star.md`**. Canonical note URLs become **`/d/n/:noteId`**; folders are addressed by **`id`**; sibling folders stay unique by **`name`** under the same parent.
...
**Remove the note parent concept** from the model after containment has been migrated into folders, relationship notes no longer depend on parent-pointer structure (**Phase 5**), product behavior treats folders as the containment surface (**Phase 6**), and **persisted slug/path addressing is retired** (boundary above). The target is not “optional parent”—it is **no parent-note field** for structure; folders are the sole mechanism for where a note lives.
...
Support **moving or reparenting a folder** within a notebook: change **`parentFolderId`** (including moving to notebook root) while keeping **folder and note placements** coherent. **Subtree membership** is keyed by **`folderId`** FKs—not by path-like slug strings (**retired before Phase 7**).

## Rationale

Moving a folder re-homes **the folder row** under a new parent. **Notes** retain **stable ids** and keep their **`folderId`** unchanged when the folder they belong to moves; only the folder hierarchy changes. **Descendant folders** nest under the moved folder row and therefore move together.

Comparison to single-note placement change:

```text
note move = one note gets a different folderId

folder move = one folder gets a different parentFolderId; all descendant folders and contained notes ride with it
```

Slug/path recomputation is **not** part of final behavior—paths are derived from **`id`** and **nested folder ids** internally if needed.

## Model and Validation

- Update the moved folder’s **`parentFolderId`** (or clear it for notebook root).
- Validate **folder name uniqueness among siblings** at the destination parent (**same rule as rename/create**—see **`ongoing/doughnut_wiki_architecture_north_star.md`**).
- Optionally validate **notebook scope** constraints (sharing, quotas) beyond structural integrity.
...
- users can move a folder to another parent or to the notebook root
- descendant folders and contained notes remain attached; structural integrity does not rely on rewritten path strings or slug columns
...
Path segments denote **nested folder sibling names**, not persisted slug chains.
...
Resolution should **prefer stable ids** (`target_note_id`) once links are persisted in cache or index. **For authoring-time resolution:**

1. **Exact title** match within notebook scope where unambiguous  
2. **Folder-qualified path**: walk folder **names** from notebook root (`Folder.name` uniqueness among siblings) then match **note title** inside the resolved folder  
3. **Relative path**: resolve from **source note's folder**, same name rules  
4. **Unique shorthand title**: match when exactly one accessible note shares that title across the notebooks the resolver considers  
5. **Unresolved**: preserve token for rendering and backlinks when missing or ambiguous  

**Do not treat path strings persisted on notes or folders** as authoritative—there are none after slug retirement.
...
- relationship-note-era content can participate in the same unified link semantics  
...
- journal folders can generate date-based titles (and exporter-friendly basenames derived from titles or patterns)  
...
Each note exports as a file named using a **basename** derived from export rules—typically **`title`** sanitized for the filesystem—with collision handling (**suffix or disambiguated path**) when two titles would collide in the same exported folder. **Do not** rely on **`note.slug`** (absent).
...
(Optional exporter keys may repeat title or importer hints; **`slug`** as persisted product identity stays out.)
...
If the target needs disambiguation, **folder-named** wiki paths may appear:
...
frontmatter title > first H1 > filename-derived title (without treating filename as persisted slug)
folder path → folder records (nested names; uniqueness among siblings within parent)
relative placement by folder hierarchy + matched note titles
```
...
- read Markdown files  
- parse frontmatter  
- create folders from folder paths (**name** uniqueness among siblings—see north star)  
- create or update notes  
- preserve IDs when available  
- derive initial **titles** from frontmatter, headings, or filename when importer rules allow—not from a resurrected **`note.slug`** column  
- parse wiki links  
...
```text
same id, relocated on disk vs internal folder graph
same parent folder + same derived title collision
different id, same importer-resolved placement
...
Doughnut should preserve stable identity through frontmatter **`id`** where possible.
...
slug/path strings or slug columns as canonical note or folder identity (Phases 1–6 historically; retired before Phase 7)
path-primary frontend routes for ordinary note viewing (prefer /d/n/:noteId — see north star)
```

## Final Architecture

The final model aligns with **`ongoing/doughnut_wiki_architecture_north_star.md`**.
...
Folder
  ...
  notebookId
  parentFolderId optional
  name      (unique among siblings under parent)
  config
...
- frontend uses **`/d/n/:noteId`** as the canonical user-facing route for ordinary note viewing  
- internal note id stays stable everywhere it matters  
...
boundary: retire persisted slugs; adopt id-canonical URLs and folder-by-id (north star)
...
```text
folder
  -> note.slug / folder.slug path model (historical phases 2–6; retired at boundary above)
      ...
      -> note properties (YAML frontmatter, rich + markdown surfaces)
        -> relationship notes as normal notes
          -> folder-first listing; remove note short details from topology
            -> slug retirement + canonical id URLs (north star)
              -> remove note parent (folders own placement)
              -> move folder (subtree moves without slug columns)
