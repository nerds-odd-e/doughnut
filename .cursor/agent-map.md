...
- Frontend tests: use `frontend/tests/`; drive mounted components; mock only the backend API via `mockSdkService()` and build payloads with `doughnut-test-fixtures/makeMe` (`unit-testing.mdc`). Stack details: `frontend-testing.mdc`.
