...
- **Clients:** The CLI requires standard Git LFS for every notebook clone, pull,
  and publish, including notes-only work.
...
Every CLI notebook clone, pull, and publish requires Git LFS, even for notes-only
work; attachment work also needs object access. Already hydrated files remain
usable offline. Pointer-only history stays small as binary versions grow,
