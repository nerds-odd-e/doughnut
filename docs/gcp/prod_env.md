...
**E2E:** Cypress uses a local **single-origin** proxy on **5173** (`e2e_test/e2e-prod-topology-proxy.mjs`). **CI** serves the built static tree from disk and forwards API paths to Spring on **9081**. **`pnpm sut`** runs the same proxy with **`E2E_PROXY_VITE_UPSTREAM`** so the browser still uses **5173** while Vite (HMR) listens on **5174**. CI `wait-on` uses **`/__e2e__/ready`**, which checks Spring from inside the proxy (avoids `HTTP_PROXY` breaking direct **9081** health requests).
