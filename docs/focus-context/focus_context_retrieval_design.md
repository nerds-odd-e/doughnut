...
Donut gathers **bounded context around one focus note** for AI features (question generation, conversation about a note, note automation, exports), for commissioned Learning Session Requests (multi-focus: retrieve per Session Item, then merge related notes into one `<related_notes>` block), and for the JSON `GET /api/notes/{note}/graph` surface.
