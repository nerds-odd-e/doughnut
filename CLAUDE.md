...
- Frontend production build outputs to `frontend/dist/`; CLI bundle is `cli/dist/doughnut-cli.bundle.mjs` (`pnpm cli:bundle`). Prod serves `/doughnut-cli-latest/doughnut` from GCS; locally `bootRun` copies the bundle into `backend/build/.../static/` (excluded from the deploy jar).
...
4. **Test observable behavior, not internal structure** — Assert what users or callers can see (responses, UI, terminal output, exit codes) by driving **stable entry points** (controllers, mounted UI, `run` / `runInteractive`, etc.). Avoid tests that mainly **mirror the codebase** (low-level functions + internal-only parameters or adapter shapes): they refactor badly and can miss real wiring. Prefer **fewer integration-style tests** when behavior spans layers, and keep assertions for **one user-visible behavior** grouped in one place when practical. **Testing strategy and layer roles:** `.cursor/rules/planning.mdc`. Stack habits: `backend-development.mdc`, `frontend.mdc`, `cli.mdc`, `e2e_test.mdc`.
