...
test('missing tracking recovers artifact source provenance from the release admission log', async (t) => {
  ...
  assert.equal(result.status, 0, result.stderr)
  assert.equal(JSON.parse(result.stdout).record.ci_sha, ciSha)
})
