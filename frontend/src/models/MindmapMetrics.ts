...
class MindmapMetrics {
  ...
  borderVector(vector: Vector): Vector {
    ...
    return {
      x: Math.round(vector.x * this.scale + dx),
      y: Math.round(vector.y * this.scale + dy),
      angle: vector.angle,
    };
  }
  ...
  straighConnection(conn: StraightConnection): StraightConnection {
    const p2p = {
      x1: conn.x1 * this.scale,
      y1: conn.y1 * this.scale,
      x2: conn.x2 * this.scale,
      y2: conn.y2 * this.scale,
    };
  ...
  coord(crd: Coord): Coord {
    return {
      x: crd.x * this.scale - this.boxWidth / 2,
      y: crd.y * this.scale - this.boxHeight / 2,
    };
  }
  ...
  linkVectors(fromO: Vector, toO: Vector): string {
    ...
    return `M ${from.x} ${from.y} C ${from.x + dist * Math.cos(from.angle)} ${
      from.y + dist * Math.sin(from.angle)
