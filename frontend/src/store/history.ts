...
class History {
  ...
  peekUndo(){
    ...
    return this.state.noteUndoHistories[this.state.noteUndoHistories.length - 1]
  }

  addEditingToUndoHistory(noteId: Doughnut.ID, textContent: Generated.TextContent) {
    this.state.noteUndoHistories.push({type: 'editing', noteId, textContent: {...textContent}});
  }
