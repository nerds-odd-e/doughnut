<template>
  <ModalWithButton v-model="show">
    <template #button>
      <span class="link-nob">
        <a role="button" :disabled="!owns" @click="show=true" :title="link.linkTypeLabel">
      ...
      <template #body>
        <div v-if="!!inverseIcon"> Source: <strong>
          <NoteTitleWithLink class="link-title" v-bind="{note:link.sourceNote}"/>
        </strong> </div>
        <LinkTypeSelect field="linkType" scopeName='link' v-model="formData.typeId" :errors="formErrors.typeId" :inverseIcon="true"/>
        <div v-if="!inverseIcon"> Target: <strong>
          <NoteTitleWithLink class="link-title" v-bind="{note:link.targetNote}"/>
        </strong> </div>
...
<script>
  ...
  export default {
    ...
    emits: ['updated'],
    components: { SvgLinkTypeIcon, ModalWithButton, LinkTypeSelect, NoteTitleWithLink },
    data() {
      return {
        show: false,
        loading: false,
    ...
    methods: {
      ...
      async deleteLink() {
        ...
        restPost(`/api/links/${this.link.id}/delete`, null, r=>this.loading=r)
          .then(res => {
            this.$emit('updated')
            this.show = false
          })
