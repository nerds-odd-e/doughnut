<template>
    <template v-if="allowPause">
        <router-link v-if="btn==='pause'" class="pause-repeat" :to="!!linkId ? {name: 'linkShow', params: {linkid: linkId}} : {name: 'noteShow', params: {noteId}}"><SvgPause/></router-link>
        <a href="#" @click.prevent v-else class="pause-repeat"><SvgResume/></a>
    </template>
