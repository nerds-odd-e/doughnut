...
describe("currentBlockAnchorIdFromAnchorPage", () => {
  ...
  it("returns null for negative or non-integer anchor page", () => {
    ...
    expect(currentBlockAnchorIdFromAnchorPage(list, -1)).toBe(null)
    expect(currentBlockAnchorIdFromAnchorPage(list, 1.5)).toBe(null)
  })

  it("earliest visible anchor is current when its top has passed viewport mid", () => {
    const list = makeMe.bookReadingTopMathsLikeAnchors()
    expect(currentBlockAnchorIdFromAnchorPage(list, 0, vp(0, 300, 600))).toBe(
      102
    )
  })
  ...
  it("earliest visible anchor whose top > mid → use previous anchor", () => {
    const list = makeMe.bookReadingTopMathsLikeAnchors()
    expect(currentBlockAnchorIdFromAnchorPage(list, 0, vp(200, 400, 600))).toBe(
      102
    )
  })
  ...
  it("earliest visible anchor with top below mid → is current", () => {
    const list = makeMe.bookReadingTopMathsLikeAnchors()
    expect(currentBlockAnchorIdFromAnchorPage(list, 0, vp(400, 600, 800))).toBe(
      103
    )
  })
  ...
  it("gap case: no visible anchor → last anchor with y0 ≤ mid (id=103)", () => {
    const list = makeMe.bookReadingTopMathsLikeAnchors()
    expect(currentBlockAnchorIdFromAnchorPage(list, 0, vp(210, 340, 380))).toBe(
      102
    )
  })
  ...
  it("gap case when viewport mid is past all anchors on page → last anchor", () => {
    const list = makeMe.bookReadingTopMathsLikeAnchors()
    expect(currentBlockAnchorIdFromAnchorPage(list, 0, vp(700, 800, 900))).toBe(
      103
    )
  })
  ...
  it("first visible has no-bbox predecessor (y0=0) whose top > mid → use no-bbox anchor", () => {
    const list = makeMe.bookReadingTopMathsLikeAnchors()
    expect(currentBlockAnchorIdFromAnchorPage(list, 0, vp(0, 50, 100))).toBe(
      101
    )
  })
  ...
  it("with pdfPageCount, skips anchors whose page_idx is beyond the document", () => {
    ...
    expect(currentBlockAnchorIdFromAnchorPage(anchors, 1, null, 2)).toBe(1)
  })
  ...
  it("with pdfPageCount, returns null when every parseable anchor is beyond the document", () => {
    ...
    expect(currentBlockAnchorIdFromAnchorPage(anchors, 1, null, 2)).toBe(null)
  })

  it("with pdfPageCount, returns null when viewport page is out of range", () => {
    const list = makeMe.bookReadingTopMathsLikeAnchors()
    expect(currentBlockAnchorIdFromAnchorPage(list, 5, null, 3)).toBe(null)
  })

  it("with invalid pdfPageCount, returns null", () => {
    const list = makeMe.bookReadingTopMathsLikeAnchors()
    expect(currentBlockAnchorIdFromAnchorPage(list, 0, null, 0)).toBe(null)
  ...
  it("partial book layout: only anchors with valid outline v1 values participate", () => {
    ...
    expect(currentBlockAnchorIdFromAnchorPage(anchors, 0)).toBe(77)
  })
  ...
  it("close-nodes: first visible partially above viewport, next within SCROLL_PADDING → use next", () => {
    ...
    expect(
      currentBlockAnchorIdFromAnchorPage(anchors, 0, vp(430, 550, 660))
    ).toBe(20)
  })
  ...
  it("on a later page with viewport showing page top, first anchor on that page is current", () => {
    ...
    expect(
      currentBlockAnchorIdFromAnchorPage(anchors, 1, vp(0, 300, 600), 2)
    ).toBe(3)
  })
})
