...
describe("DragListner", () => {
  const currentTarget = { getBoundingClientRect: jest.fn().mockReturnValue({ width: 1000, height: 1000, top: 0}) }

  test("mouse move only", async () => {
    const wrapper = mount(DragListner, {propsData: { modelValue: {x: 10, y: 20}}});
    await wrapper.find("div").trigger("pointermove", {clientX: 100, clientY: 200, currentTarget});
  ...
  test("touch move", async () => {
    const wrapper = mount(DragListner, {propsData: { modelValue: {x: 10, y: 20}}});
    await wrapper.find("div").trigger("pointerdown", { clientX: 100, clientY: 200})
    await wrapper.find("div").trigger("touchmove", { changedTouches: [{clientX: 1000, clientY: 2000, currentTarget}]})
    expect(wrapper.emitted()['update:modelValue'][0][0]).toEqual({x: 910, y: 1820})
  });
