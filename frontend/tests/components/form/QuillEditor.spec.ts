...
describe("QuillEditor.vue", () => {
  ...
  it("does not put br inside first list item when pasting HTML with empty paragraph before list", async () => {
    wrapper = mount(QuillEditor, {
      props: { modelValue: "", readonly: false },
      attachTo: document.body,
    })
    ...
    if (quillInstance) {
      ...
      // HTML from ChatGPT or similar apps: empty paragraph with br before list
      const inputHtml = `<p style="margin: 0.0px 0.0px 12.0px 0.0px; font: 12.0px 'Times New Roman'"><span></span><br></p>
<ul style="list-style-type: disc">
  <li style="margin: 0.0px 0.0px 12.0px 0.0px; font: 12.0px 'Times New Roman'"><span>No rendaku</span></li>
  <li style="margin: 0.0px 0.0px 12.0px 0.0px; font: 12.0px 'Times New Roman'"><span>No dakuten</span></li>
</ul>`
      ...
      // The output HTML should not have a list item that starts with <br>
      // Bug pattern: <li...><br>... followed by text
      expect(outputHtml).not.toMatch(/<li[^>]*>\s*<br/)

      // The list items should contain the proper text
      expect(outputHtml).toContain("No rendaku")
      expect(outputHtml).toContain("No dakuten")
    }
  })
})
