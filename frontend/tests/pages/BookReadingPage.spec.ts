...
describe("BookReadingPage", () => {
  ...
  describe("Reading control panel", () => {
    ...
    it("snaps back to last valid position on first boundary crossing for unread block with direct content (Phase 5)", async () => {
      const wrapper = await mountLoadedBookWithBlocks(notebookId)
      const { spy } = pdfScrollRestoreSpy(wrapper)
      ...
      // Emit viewport inside Section 1 with an explicit reading position to save
      const pdf = wrapper.findComponent(PdfBookViewer)
      await withFakeTimers(async () => {
        pdf.vm.$emit("viewportAnchorPage", {
          anchorPageIndexZeroBased: 0,
          viewport: { top: 0, mid: 60, bottom: 200 },
          pagesCount: 10,
          readingPosition: { pageIndexZeroBased: 0, normalizedTop: 150 },
        })
        await vi.advanceTimersByTimeAsync(CURRENT_BLOCK_ANCHOR_DEBOUNCE_MS)
        await flushPromises()
      })

      // Cross the boundary into Section 2
      await emitViewportAndSettleCurrentBlock(wrapper, {
        anchorPageIndexZeroBased: 0,
        viewport: { top: 0, mid: 500, bottom: 1000 },
        pagesCount: 10,
    ...
    it("does not snap back when selected block already has a disposition (Phase 5)", async () => {
      vi.spyOn(
        NotebookBooksController,
        "getNotebookBookReadingRecords"
      ).mockResolvedValue(
      ...
      // Use dwell-based auto-selection: emit viewport inside Section 1, wait for dwell
      const pdf = wrapper.findComponent(PdfBookViewer)
      vi.useFakeTimers()
      try {
        pdf.vm.$emit("viewportAnchorPage", {
    ...
    it("does not snap back for no-direct-content blocks (auto-read path) (Phase 5)", async () => {
      vi.spyOn(
        NotebookBooksController,
        "putNotebookBookBlockReadingRecord"
      ).mockResolvedValue(
