...
Given(
  ...
  (externalIdentifier, data: DataTable) => {
...
When(
  "I try to create a note belonging to {string}:",
  (noteTitle: string, data: DataTable) => {
    expect(data.hashes().length).to.equal(1);
    cy.jumpToNotePage(noteTitle);
    cy.clickAddChildNoteButton();
...
Then(
  "I should see {string} in breadcrumb with add sibling button",
  (noteTitles: string) => {
    cy.pageIsNotLoading();
...
Then(
  ...
  (notePath: NotePath, data: DataTable) => {
    cy.navigateToNotePage(notePath).then(() =>
      cy.expectNoteCards(data.hashes())
    );
  }
);
...
When(
  "I create a sibling note of {string}:",
  (noteTitle: string, data: DataTable) => {
    expect(data.hashes().length).to.equal(1);
    cy.findNoteTitle(noteTitle);
...
When(
  "I open the {string} view of note {string}",
  (viewType: string, noteTitle: string) => {
  ...
  (noteTitles: string, parentNoteTitle: string) => {
    ...
    cy.withinMindmap().then((cards) => {
      titles.forEach((noteTitle: string) => {
        cy.distanceBetweenCardsGreaterThan(
          cards,
          parentNoteTitle,
          noteTitle,
          100
...
When(
  "I ask for a description suggestion for note {string}",
  (noteTitle: string) => {
...
Given(
  "I ask to complete the description for note {string}",
  (noteTitle: string) => {
    cy.aiSuggestDescriptionForNote(noteTitle);
  }
);
...
Then(
  "I should see that the open AI service is not available in controller bar",
  () => {
...
When(
  "I select the correct option {questionoption}",
...
Then(
  "I should see option {questionoption} turn green",
  (option: QuestionOption) => {
...
Then(
  "I should see option {questionoption} turn red",
  (option: QuestionOption) => {
