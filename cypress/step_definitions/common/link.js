...
And(
  ...
  (noteTitlesAsString, searchKey) => {
    ...
    cy.expectExactLinkTargets(
      noteTitlesAsString.commonSenseSplit(",").map((i) => i.trim())
    );
  }
);

And(
  "I should see note cannot be found when searching {string}",
  (searchKey) => {
    cy.searchNote(searchKey);
...
Then(
  ...
  (noteTitle, linkType, targetNoteTitles) => {
    cy.findByText(targetNoteTitles.commonSenseSplit(",").pop(), {selector: '.link-title'});
...
Then(
  "I should see {string} has no link of type {string}",
  (noteTitle, linkType) => {
