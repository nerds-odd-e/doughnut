...
public class Authorization extends ModelForEntity<User> {
    ...
    public void assertPotentialReadAuthorization(Notebook notebook) throws NoAccessRightException {
        if (!hasReadAuthority(notebook.getHeadNote()) && modelFactoryService.bazaarNotebookRepository.findByNotebook(notebook) == null) {
            throw new NoAccessRightException();
        }
    }
