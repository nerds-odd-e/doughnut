...
/**
 * copied from
 * https://github.com/Lambdua/openai4j/blob/5e2814a6e48d40c0f76a39b3d7e7a2469ba36ed3/api/src/main/java/com/theokanning/openai/function/FunctionDefinition.java#L115
 * To fix the problem of missing `strict` field
 */
...
public class FunctionDefinition {
  ...
  /**
   * This feature works with all models that support tools, including all models gpt-4-0613 and
   * gpt-3.5-turbo-0613 and later. When Structured Outputs are enabled, model outputs will match the
   * supplied tool definition.
   */
  private Boolean strict;

  /**
   * parameters definition by class schema ,will use {@link JsonSchemaGenerator} to generate json
   * schema
   */
  private Class<?> parametersDefinitionClass;

  /**
   * The parameters the functions accepts.Choose between this parameter and {@link
   * #parametersDefinitionClass} This parameter requires you to implement the
   * serialization/deserialization logic of the JSON schema yourself
   */
  private Object parametersDefinition;

  /**
   * Function executor,if set {@link #parametersDefinitionClass},the executor type must {@link
   * #parametersDefinitionClass}. <br>
   * Else executor parameter type must {@link #parametersDefinition} JsonNode type 可以为null
   */
  ...
  public static class Builder<T> {
    ...
    public FunctionDefinition.Builder<T> parametersDefinitionByClass(
        Class<T> parametersDefinitionClass) {
    ...
    public FunctionDefinition build() {
      ...
      if (parametersDefinition != null && parametersDefinitionClass != null) {
        throw new IllegalArgumentException(
            "parametersDefinitionClass and parametersDefinition can't be set at the same time,please set one of them");
      }
