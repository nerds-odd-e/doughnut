...
@Service
public class AdminDataMigrationProgressService {

  @PersistenceContext private EntityManager entityManager;

  private final AdminDataMigrationProgressRepository progressRepository;
  private final NoteRepository noteRepository;
  private final JdbcTemplate jdbcTemplate;

  public AdminDataMigrationProgressService(
      AdminDataMigrationProgressRepository progressRepository,
  ...
  public Optional<AdminDataMigrationProgress> find(String stepName) {
   ...
   */
  @Transactional
  public AdminDataMigrationProgress startOrResume(String stepName, int totalCount) {
    Optional<AdminDataMigrationProgress> existing = progressRepository.findByStepName(stepName);
    if (existing.isPresent()) {
      AdminDataMigrationProgress p = existing.get();
      ...
      if (p.getStatus() == WikiReferenceMigrationStepStatus.PENDING) {
        p.setStatus(WikiReferenceMigrationStepStatus.RUNNING);
      }
      return progressRepository.save(p);
    }
    AdminDataMigrationProgress created = new AdminDataMigrationProgress();
  ...
  public List<Integer> pendingNoteIdsOrdered(String stepName, List<Integer> orderedCandidateIds) {
    Optional<AdminDataMigrationProgress> opt = progressRepository.findByStepName(stepName);
    if (opt.isEmpty()) {
      return List.copyOf(orderedCandidateIds);
    }
    AdminDataMigrationProgress p = opt.get();
    if (p.getStatus() == WikiReferenceMigrationStepStatus.COMPLETED) {
      return List.of();
    }
  ...
  public void recordBatchSuccess(
    ...
    AdminDataMigrationProgress p = progressRepository.findByStepName(stepName).orElseThrow();
    Note last =
        noteRepository
  ...
  public void markFailed(String stepName, String message) {
    ...
    int updated =
        jdbcTemplate.update(
            "UPDATE admin_data_migration_progress SET status = ?, last_error = ?, updated_at = ?"
                + " WHERE step_name = ?",
            ...
            stepName);
    if (updated != 1) {
      throw new IllegalStateException("No admin_data_migration_progress row for step: " + stepName);
    }
    entityManager.clear();
  }
}
