...
public class LinkViewedByUser {
  ...
  @Getter @Setter private NoteWithPosition sourceNoteWithPosition;
  ...
  public static LinkViewedByUser from(Link link, UserModel user) {
    ...
    linkViewedByUser.setSourceNoteWithPosition(
        new NoteViewer(userEntity, sourceNote).jsonNoteWithPosition(sourceNote));
    linkViewedByUser.setTargetNoteWithPosition(
        new NoteViewer(userEntity, targetNote).jsonNoteWithPosition(targetNote));
