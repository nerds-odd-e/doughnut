  ...
  class FetchWikiData {
    ...
    private String getEntityDataJsonSearch(String search) {
      return "{\"searchinfo\":{\"search\":\"" + search
          ...
          + "\"http://www.wikidata.org/entity/Q64\",\"label\":\"" + search
          + "\",\"description\":\"federal state, capital and largest city of Germany\""
          + ",\"match\":{\"type\":\"label\",\"language\":\"en\",\"text\":\"" + search
          + "\"}}],\"search-continue\":10,\"success\":1}";
    }

    private String getEntityDataJsonSearchEmpty(String search) {
      return "{\"searchinfo\":{\"search\":\"" + search + "\"},\"search\":[],\"search-continue\":10,\"success\":1}";
    }
    ...
    void serviceNotAvailable() throws IOException, InterruptedException {
      ...
      BindException exception = assertThrows(BindException.class, () -> controller.fetchWikiDataByID("Q1"));
    ...
    void serviceNotAvailableAtSearchWikidata() throws IOException, InterruptedException {
      Mockito.when(httpClientAdapter.getResponseString(any())).thenThrow(new IOException());
      BindException exception = assertThrows(BindException.class, () -> controller.fetchsWikiDataBySearch("berlin"));
      assertThat(exception.getErrorCount(), equalTo(1));
    }

    @Test
    void shouldFetchDataAtSearchWikidata() throws IOException,
        InterruptedException, BindException {
      Mockito.when(httpClientAdapter.getResponseString(any()))
          .thenReturn(getEntityDataJsonSearch("berlin"));
      controller.fetchsWikiDataBySearch("berlin");
      Mockito.verify(httpClientAdapter)
          .getResponseString(any());
    }

    @Test
    void shouldParseAndGetAtSearchWikidata() throws IOException, InterruptedException,
        BindException {
      Mockito.when(httpClientAdapter.getResponseString(any()))
          .thenReturn(getEntityDataJsonSearch("berlin"));
