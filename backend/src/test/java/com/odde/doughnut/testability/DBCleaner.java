...
public class DBCleaner implements BeforeEachCallback {
    ...
    private void truncateAllTables() {
        withEntityManager(entityManager -> getTableNames(entityManager).forEach(t->truncateTable(t, entityManager)));
    }
    ...
    private List<String> getTableNames(EntityManager manager) {
        ...
        return entities.stream().map(EntityType::getName).collect(Collectors.toList());
    }
