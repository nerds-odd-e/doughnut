...
@ActiveProfiles("test")
@Transactional
class NoteLevelDummySequenceSkipConversionTest {

  private static final String MIGRATION_SQL =
      "/db/migration/V300000254__convert_note_level_dummy_sequence_skips.sql";
  private static final String GATE_PLACEHOLDER = "${dummy_note_sequence_skip_convert}";
  ...
  void seedSelectionSiblings() {
    ...
    dummyTracker = makeMe.aMemoryTrackerFor(dummyNote).removedFromTracking().please();
    spellingTracker = makeMe.aMemoryTrackerFor(dummyNote).spelling().removedFromTracking().please();
    commissionedTracker =
        makeMe.aMemoryTrackerFor(dummyNote).commissioned().removedFromTracking().please();
    propertyTracker =
        makeMe.aMemoryTrackerFor(dummyNote).propertyKey("topic").removedFromTracking().please();
    ...
    existingSkipTracker = makeMe.aMemoryTrackerFor(existingSkipNote).removedFromTracking().please();
    makeMe.anAssimilationSequenceSkipFor(existingSkipNote).please();

    stillTracked = makeMe.aMemoryTrackerFor(makeMe.aNote().please()).please();
  }

  @Test
  void defaultGateDoesNotConvertDummyNoteSkip() {
    runConversion("1=0");
  ...
  class WhenGateIsEnabled {
    ...
    @Test
    void convertsNoteLevelDummyUnderstandingSkip() {
