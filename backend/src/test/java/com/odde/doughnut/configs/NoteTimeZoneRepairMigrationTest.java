 ...
 * db/migration/V300000235__repair_tz_skewed_note_created_at.sql): with the no-op default
...
@ActiveProfiles("test")
@Transactional
class NoteTimeZoneRepairMigrationTest {
  ...
  void doesNotTouchRowsWhenPlaceholderDefaultsToNoOp() throws Exception {
    String migrationSql =
        StreamUtils.copyToString(
            new ClassPathResource("db/migration/V300000235__repair_tz_skewed_note_created_at.sql")
                .getInputStream(),
    ...
    try {
      ...
      try (PreparedStatement insert =
          connection.prepareStatement(
              "INSERT INTO note (title, created_at, updated_at) VALUES ('tz-repair-test-note', ?, ?)")) {
        insert.setTimestamp(1, new Timestamp(originalEpochMillis));
        insert.setTimestamp(2, new Timestamp(originalEpochMillis));
      ...
      try (PreparedStatement select =
          connection.prepareStatement("SELECT created_at FROM note WHERE id = ?")) {
