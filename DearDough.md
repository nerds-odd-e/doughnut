...
## DD-042 — Cross-stack concept removal missed frontend/test-fixture references caught only by type checker

A slice removing a concept across the full stack (backend production
code, backend tests, frontend, generated API, test fixtures) focused
its cleanup on the backend and missed three frontend/test-fixture
references: a `"delete note"` case left in `NoteUndoButton.vue`'s switch
after its union type was narrowed, two unused imports in adapted test
files, and a `deletedAt` property still in the test-fixture
`NoteBuilder.ts` default object. The coordinator's `format:changed`
run (which includes `vue-tsc --noEmit` type checking) caught all
three before staging; the coordinator fixed them directly.

### Occurrences

- Execution: SEED-009 story 31 / quick/120-recover-legacy-notes-and-rebuild-git / 930e948f3d
  - Timestamp: 2026-09-13T20:10:00+08:00
  - Tool: Cursor
  - Model: GLM 5.2
  - Open Dough release: 0.3.16
  - Evidence: slice 11b implementation agent returned with
    `backend:verify` and backend/frontend suites green, but the
    coordinator's `format:changed` run reported four TypeScript
    errors (`TS2678` stale switch case, two `TS6133` unused imports,
    `TS2353` unknown property `deletedAt`). The coordinator fixed
    `NoteUndoButton.vue`, `NoteNewForm.wikidata.spec.ts`,
    `SearchDialog.actions.spec.ts`, and `NoteBuilder.ts` before
    staging; the commit passed `vue-tsc` clean.
  - Observed effect: one extra coordinator round-trip to fix three
    files the implementation agent missed; no lost work and no
    contaminated commit, since `format:changed` caught the errors
    before staging.
  - Inference: when removing a concept across the full stack, the
    implementation agent should grep all layers (backend `src/main`,
    backend `src/test`, `frontend/src`, `frontend/tests`, generated
    packages, test fixtures) for references to the removed concept,
    not just the backend production code; the `format:changed` type
    check is a safety net, not a substitute for layer-complete cleanup.
