...
this project's `pnpm sut` isolated-SUT bring-up — which derives a Unix domain
...
- Execution: quick-107-verify-publication-receiver-in-bulk / d37290fac9
  ...
  - Evidence: worktree path
    ...
    argument` during `pnpm sut` bring-up, traced by a reverted one-line debug
    change to un-ignore stdio, and independently reproduced by the
    coordinator via `pnpm sut:healthcheck` reporting a missing E2E
    allocation.
...
commit's lint hook, and `pnpm sut:healthcheck`) runs `pnpm --frozen-lockfile
...
- Execution: quick-107-verify-publication-receiver-in-bulk / d37290fac9
  ...
  - Evidence: `pnpm-lock.yaml` diff showing `@biomejs/biome` `2.5.12` →
    ...
    reappeared after (1) the slice 1 implementation agent's `pnpm sut`
    attempts, (2) the coordinator's `pnpm sut:healthcheck` call, and (3) the
