...
## DD-099 — `register-push` accepted a hand-typed revision that does not exist in the repository

`ci-mailbox.mjs register-push OBSERVER SHA` registered a full SHA the coordinator typed from memory (a mistyped expansion of an abbreviated hash) without checking that the revision exists in the execution checkout. The observer then reported `CI_COVERAGE_UNAVAILABLE` for a revision no push ever produced and listed it as `uncovered` at shutdown.

### Occurrences

- Execution: quick/010-change-proportional-note-save / 91bf241404
  - Timestamp: 2026-09-22T03:36:00Z (within a minute after the 03:35:13Z push of `91bf241404`; exact second not recorded)
  - Tool: Claude Code
  - Model: claude-fable-5-1
  - Open Dough release: modified; revision 2ef78c3990; base 0.3.27
  - Evidence: registration receipt for `91bf241404b3c6e6ac5f4c9f9c6bd8b3a3d8d2c1` (nonexistent) followed by a corrected registration of `91bf241404bf9b20c86bca49e252f13996d53a21` from `git rev-parse HEAD`; later `CI_COVERAGE_UNAVAILABLE` for the nonexistent SHA; stop receipt `unproved: [{sha: 91bf241404b3c6..., state: uncovered}]`.
  - Observed effect: one spurious coverage event to triage and one permanently uncovered entry; no product impact.
  - Inference: the script could refuse a SHA absent from the checkout (`git cat-file -e`), and the delivery step could instruct deriving the SHA from Git rather than transcribing it. Coordinator error, but cheaply preventable in the script.

## DD-100 — Post-change refactor passes routinely took two to five times longer than the implementation slice they followed

`dough-post-change-refactor` has no time or scope budget comparable to the execution slice's 5-minute target and 10-minute limit. In a nine-slice execution the refactor agents reported 6, 6, 12, 12, 15, 25, 12, 15 minutes of active time against implementation reports of about 6, 5, 6, 4, 5, 7, 8, 8 minutes, and several passes performed class extractions, renames, test-base moves and full-suite reruns.

### Occurrences

- Execution: quick/010-change-proportional-note-save / 91bf241404
  - Timestamp: 2026-09-22T04:20:00Z to 2026-09-22T05:50:00Z (slice 5 to slice 8 refactor passes; slice 6's ran roughly 04:52Z to 05:08Z)
  - Tool: Claude Code
  - Model: claude-fable-5-1
  - Open Dough release: modified; revision 2ef78c3990; base 0.3.27
  - Evidence: refactor reports' "approximate active elapsed time" lines (slice 6: about 25 minutes, 199k subagent tokens versus the implementer's 139k; slice 5: about 15 minutes; slice 8: about 15 minutes); the resulting edits shipped in `e24442f18e`, `929aad7e94`, `7ce390e463`, `a6f5223fbb`.
  - Observed effect: the refactor pass, not implementation, dominated wall-clock per slice from slice 5 on. The edits were accepted and removed duplicated rules (README rule, folder-prefix walk) and a 309-line test class; no rework followed.
  - Inference: not shown to be waste, since the results were kept, but the skill's "smallest complete set" has no sizing rule, so a pass may grow to a slice of its own. A budget or a "stop and report the rest as a candidate" rule comparable to slice sizing would let the coordinator decide instead of the agent.

## DD-101 — A plan's "existing owners" list asserted an operation is routed through an owner without reading its entry point

The 010 plan (and the seed's promise 1) stated that folder README edits go through `AcceptedWebChangeService` today. The README endpoints save and flush directly and never enter that owner. The claim was only tested at slice 6, when the implementer looked for the controller path, and the retrospective turned the gap into a correction plan.

### Occurrences

- Execution: quick/010-change-proportional-note-save / 91bf241404
  - Timestamp: 2026-09-22T05:00:00Z (slice 6 implementation report; the plan was written 2026-09-22 before execution)
  - Tool: Claude Code
  - Model: claude-fable-5-1
  - Open Dough release: modified; revision 2ef78c3990; base 0.3.27
  - Evidence: `.planning/quick/010-change-proportional-note-save/PLAN.md` "Included: all web changes that go through `AcceptedWebChangeService` today (... folder README edits ...)"; `NotebookFolderController.updateFolderReadmeContent` and `NotebookController.updateNotebookReadmeContent` call `entityPersister.save` and `flush` with no `apply`; correction plan `.planning/quick/011-readme-edits-through-accepted-change-owner`.
  - Observed effect: slice 7's README derivation could only be proven through the owner's `apply` entry point; the product behavior the story listed does not exist yet.
  - Inference: when a plan lists "operations that go through X today", planning inspection should cite the entry point for each, not the service's callers alone. Cost was bounded (one delegated check, one follow-up plan), but the story's promise was overstated.

## DD-102 — Running the owner's broad class-pattern regression on every slice caught cross-slice test fallout twice

Each slice's delegation asked for a focused run and then one broad `--tests 'com.odde.donut.controllers.NotebookGit*'` run (about 375 to 395 tests, roughly a minute). In slice 3 that run found four before-snapshot drift tests the plan's learning had not counted; in slice 8 the full suite found a stray-row migration test that the focused set could not exercise.

### Occurrences

- Execution: quick/010-change-proportional-note-save / 91bf241404
  - Timestamp: 2026-09-22T03:55:00Z (slice 3 report) and 2026-09-22T05:45:00Z (slice 8 report)
  - Tool: Claude Code
  - Model: claude-fable-5-1
  - Open Dough release: modified; revision 2ef78c3990; base 0.3.27
  - Evidence: slice 3 report "4 failures, all in tests outside my slice list"; slice 8 report "First run: 2575 tests completed, 1 failed: NotebookFollowsFolderContainmentMigrationTest"; both fixed before delivery.
  - Observed effect: no CI failure was delivered for any of the eight code commits on the story branch.
  - Inference: a useful practice when a slice changes a shared owner; delegation guidance could name it as the default when the slice touches an owner with many callers, since the full backend suite here costs about one minute.
