...
Restored to the shared findings log. The `.claude/skills` startup path and
checkout-bound runtime discovery contract come from published Open Dough
v0.3.26 guidance, which Donut uses unchanged with the default GitHub Actions
adapter. Donut’s gitignored, shell-generated skill layout is relevant occurrence
context, but does not establish a separate product-owned observer responsibility.

A fresh export of Donut revision `8766adefee` reproduced `MODULE_NOT_FOUND`
through the documented Claude path; the tracked `.agents/skills` control printed
`CI_OBSERVER`. Resolution remains unconfirmed. The v0.3.27 CLI-entry fix for
DD-065 does not itself supply a missing runtime directory. Keep this as shared
runtime-discovery/installation integration feedback, preserving the local layout
evidence below. The two Donut stories incorrectly created as SEED-038 were
withdrawn after this ownership check; no product replacement is queued.
...
## DD-087 — A plan named an unpublished commit as the historical comparison baseline

Plan 004 named `1204b33d3f48da94ad495e1a0c9beb6054b68aa2` as the earlier revision for its
reconstructed save comparison. The object existed neither locally nor on `origin`. Verifying it
before the first dispatch caught it, and the recorded product baseline `b5cad203d1` was substituted.

### Occurrences
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  - Timestamp: unknown (2026-09-21)
  - Tool: Claude Code
  - Model: claude-opus-5
  - Open Dough release: 0.3.26
  - Evidence: `git cat-file -t 1204b33d` -> `bad object`; plan 004 "Verified earlier revision"
  - Observed effect: dispatching slices 1-3 against the named revision would have failed; caught before any delegation
  - Inference: the earlier plan recorded a local, never-pushed revision; plans should cite only published revisions

## DD-088 — A slice's proof depended on infrastructure a later slice would build

Slice 1's proof - import the fixture into both revisions and compare exports - needed a booted
isolated stack and a seeding spec, which slice 2 created. The slice returned an oversized report
and its proof was relocated to slice 3.
...
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  ...
  - Model: claude-opus-5
  - Open Dough release: 0.3.26
  - Evidence: plan 004 slice 1 refinement note; slice 1 oversized-slice report
  - Observed effect: about 30 active minutes against a ~5-minute budget, then a plan refinement

## DD-089 — A measurement harness carried a multi-megabyte payload through the browser, inflating every measured save

The comparability export was passed from the browser into a `cy.task` as base64 (3.4 MB for the
11,000-note fixture). With 12.5 MB of attachments it produced 4-7 s debounces and failed reloads,
which exposed it in slice 13 - after slice 3 had reported a comparison using the same harness.
...
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  ...
  - Model: claude-opus-5
  - Open Dough release: 0.3.26
  - Evidence: plan 004 slice 13 "Harness defect found and fixed"; slice 15 result
  - Observed effect: an extra re-measurement slice; the corrected ratio (about 1.35x) barely moved
  - Inference: measuring the harness's own overhead in the slice-2 pilot would have caught it before any comparison

## DD-090 — Volume-driven fixture sweeps overran a ~5-minute slice budget while the subdivision trigger never fired

Slices 5-7 each changed 13-19 files by the same two patterns and ran 12-20 active minutes (slice 2
ran about 25). The plan's trigger was "subdivide if edits differ", which did not apply, and splitting
would have left the tree non-compiling between leaves.
...
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  ...
  - Model: claude-opus-5
  - Open Dough release: 0.3.26
  - Evidence: plan 004 sizing notes for slices 2, 5, 6 and 7
  - Observed effect: repeated overrun reports needing coordinator judgment; the expectation was revised to 15-20 minutes mid-run
  - Inference: sizing for mechanical sweeps should scale with call-site count

## DD-091 — Coordinator briefs stated unverified framework behavior as fact; subagents disproved it from code

Slice 10's brief expected a failed Flyway Java migration on `ApplicationReadyEvent` to leave the app
running; the agent traced `SpringApplication.run` closing the context and the JVM exiting (a restart
loop under instance autohealing). The no-squash plan claimed `flyway.repair()` would mark the deleted
newest migration deleted; the agent showed Flyway treats it as a future migration until a newer one ships.
...
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  ...
  - Evidence: plan 004 slice 10 "Production behavior if the check fails"; slice 12 "Correction to the reasoning above"
  - Observed effect: release 2's instructions changed to keep both servers; the plan was corrected
  - Inference: both were caught because the briefs also asked agents to verify from code; state such expectations as claims to check

## DD-092 — A plan embedded an optional migration squash that the owner had not asked for and then declined

The cleanup followed the `db-migration` skill's baseline-squash procedure: a tip placeholder, a
migration freeze, and a gate on every long-lived database. After release 2 the owner asked "what is
the squash? We don't need a squash." The placeholder shipped in release 2 was then deleted and the
freeze lifted.
...
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  ...
  - Tool: Claude Code
  - Model: claude-opus-5
  - Open Dough release: 0.3.26 when planned; declined under 0.3.27
  - Evidence: plan 004 owner decision 4; slices 11 and 12
  - Observed effect: one shipped-then-retired migration (version 300000339 now reserved), a freeze imposed and lifted, extra slices
  - Inference: optional heavyweight procedures in a plan need explicit owner confirmation

## DD-093 — Refactor passes reported completion while a required check was unmet or the on-disk proof was stale

Slice 17's refactor reported `## REFACTOR COMPLETE` while disclosing a test file it had pushed to
284 lines, over the 250-line check. Slice 10's final on-disk test results came from a deliberately
failing falsifiability run, not the final tree. Related to ODF-051, but here the gap was disclosed
or stale rather than misreported, so it is recorded separately.
...
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  ...
  - Tool: Claude Code
  - Model: claude-opus-5
  - Open Dough release: 0.3.26 (slice 17); 0.3.27 (slice 10)
  - Evidence: plan 004 slice 17 refactor outcome; slice 10 proof note
  - Observed effect: the coordinator sent slice 17 back and re-ran slice 10's suite on the final tree (green) before committing

## DD-094 — The CI host-bridge readiness probe exited 0 with no receipt, leaving every push of the execution unobserved

`node .claude/skills/dough-execute-plan/scripts/ci-mailbox.mjs probe` in the execution worktree printed
nothing - no `CI_OBSERVER` receipt - and no `CI_MONITOR_READY` context appeared, so no observer was
armed. Distinct from ODF-085: the runtime path existed and ran.
...
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  ...
  - Model: claude-opus-5
  - Open Dough release: 0.3.26
  - Evidence: plan 004 "CI observation: unavailable"
  - Observed effect: about 30 pushes, including the source of two production releases, went unobserved; proof rested on local suites
  - Inference: cause unknown; hook registration may not have been active in this session

## DD-095 — Checking release tags and the instance-group rollout before a release exposed an unsafe cutover (useful practice)

`git tag --contains` showed the native storage and backfill had never been released, so the next
release was production's cutover. `create-app-mig.sh` (`--size 2`) and the rolling replace
(`--max-surge 0 --max-unavailable 1`) meant old and new servers would overlap. The handoff added a
resize to one server; release 2 deliberately kept two after a traced failure mode showed one would
turn a refused drop into an outage.
...
- Execution: Plan 004 (SEED-034#story-4), first implementation commit `19e5d5d880`
  ...
  - Tool: Claude Code
  - Model: claude-opus-5
  - Open Dough release: 0.3.26 and 0.3.27
  - Evidence: plan 004 "Release facts established 2026-09-21" and release handoffs 1-2
  - Observed effect: both releases passed their production checks with no overlap of old and new writers
