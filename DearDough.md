...
## DD-066 — Requiring a demonstrated failure for tests expected to pass immediately exposed a vacuous refusal fixture

For proof-only Behavior slices (no production change expected), the coordinator
required the implementer to temporarily break the guarded behavior and show the
new test fails. That check exposed a fixture whose test passed for the wrong
reason, which a green run alone would have accepted.

### Occurrences

- Execution: SEED-025 story 1 / quick/138-cross-notebook-relationship-reduction, first implementation commit 700f8775b6
  - Timestamp: 2026-09-17, evening +08:00 (slice 5 implementation)
  - Tool: Claude Code
  - Model: claude-opus-5
  - Open Dough release: modified; revision f5ab5b1b18; base unknown
  - Evidence: slice 5 handoff; a subscription-based "not editable" source notebook passed with the editability check disabled; `NotebookGitWebRelationReduceControllerTest.refusedReduceIntoReadOnlySourceNotebookLeavesBothNotebooksUnchanged` uses a bazaar-shared notebook and fails when the check is disabled; slice 6 applied the same check (committing every opened notebook made its drift test fail); plan 138 Learnings.
  - Observed effect: the vacuous proof was caught before delivery. Slice 5 took about 15 minutes, past the 10-minute hard limit, mostly spent diagnosing the fixture.
  - Inference: worth keeping as a default delegation requirement for "no production change expected" slices. The overrun came from the diagnosis, not from slice size, so the coordinator kept the proven work instead of reverting and refining it.

## DD-067 — Plan key examples used an unverified wiki-link notation for cross-notebook links

The story seed and plan wrote cross-notebook links as `[[Astronomy/Moon]]` and
the reduced value as `[[Space topics/Earth]]`. The product's notation is
`Notebook:Title`; a slash is a folder path inside the same notebook, and the
qualify rule keeps the visible text (`[[Space topics:Earth|Earth]]`).

### Occurrences

- Execution: SEED-025 story 1 / quick/138-cross-notebook-relationship-reduction, first implementation commit 700f8775b6
  - Timestamp: 2026-09-17, evening +08:00 (slice 1 implementation)
  - Tool: Claude Code
  - Model: claude-opus-5
  - Open Dough release: modified; revision f5ab5b1b18; base unknown
  - Evidence: slice 1 handoff; `RelationControllerTests.crossNotebookMove_rewritesMovedNotesOutgoingUnqualifiedLinks` expects `[[OldNb:X|X]]`; the plan examples were corrected in 700f8775b6; SEED-025 key example 1 still uses slash notation.
  - Observed effect: the implementer caught it test-first at little cost. Had it been followed literally, the fixture would have described a same-notebook folder path and the slice 4 assertion would have been wrong.
  - Inference: when refinement or planning writes concrete Portable markdown into key examples, checking it against an existing test of the same notation is cheap.

## DD-068 — The worktree-isolation guard refused several coordinator shell commands in Story Branch Mode

After the session entered the execution worktree, the Claude Code guard refused
Bash commands whose program arguments or text it could not prove stayed inside
the worktree. Examples: a `node` path built from `$PWD`, `sed -i` on a path
held in a shell variable, a Python heredoc whose text contained `notebookGit`
paths plus `git add`, and a compound `git push … ; SHA=$(git rev-parse …)`.

### Occurrences

- Execution: SEED-025 story 1 / quick/138-cross-notebook-relationship-reduction, first implementation commit 700f8775b6
  - Timestamp: 2026-09-17, evening +08:00 (CI arming and slice 1–2 delivery)
  - Tool: Claude Code
  - Model: claude-opus-5
  - Open Dough release: modified; revision f5ab5b1b18; base unknown
  - Evidence: five refused Bash calls in the coordinator conversation, each retried successfully as a plain command with literal absolute paths or as an Edit tool call.
  - Observed effect: one extra round-trip per refusal; no state was affected.
  - Inference: coordinators on this host should default to literal absolute paths, one git operation per command, and Edit/Write for plan text during delivery.
