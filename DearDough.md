...
## DD-004 — Backend-test leftover `.worktree.local.lock` blocks retirement; migrateTestDB reclaim makes another stale owner

After a supported backend test or `migrateTestDB` in a linked worktree, the
invocation PID remains in `.worktree.local.lock` even though the process has
exited. `pnpm worktree:retire --check` correctly refuses that stale record.
The next backend command can reclaim it, but that reclaim writes a new owner
PID and leaves the same leftover. Operator removal of the lock directory is
what makes an idle snapshot until quick/102 ships the release path.

### Occurrences

- Execution: SEED-009 Story 11 / quick/103-publish-concepts-into-implied-folder / 8af0939de5
  - Tool: Cursor
  - Model: Cursor Grok 4.6
  - Open Dough release: 0.3.8
  - Evidence: after `pnpm backend:test_only` in `/Users/terryyin/git/doughnut-103-implied-folder`,
    `--check` refused "stale or unverifiable backend worktree owner record
    (pid 82914 in .worktree.local.lock; not reclaimed)" with that PID not
    running. `migrateTestDB` reclaimed 82914 then left pid 32655 in
    `owner.pid`. `trash .worktree.local.lock` then `--check` reported idle
    and retirement dropped `doughnut_wt_f928d1c08ce04061b489adf1e3e0e7e3_test`.
  - Observed effect: one extra migration plus a second stale-lock refusal
    before the worktree could be dropped.
  - Inference: do not use `migrateTestDB` to clear ownership for retirement;
    remove the leftover backend lock after the command has exited, then retire.
    Product correction remains quick/102.
