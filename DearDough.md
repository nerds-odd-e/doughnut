...
## DD-052 — GSD STATE.md progress hook fires on Open Dough plan writes

During dough-execute-plan Story Branch Mode, every coordinator edit to an
Open Dough PLAN under `.planning/quick/` injected a GSD reminder to update
`.planning/STATE.md`. This repository's coexistence rule forbids Open Dough
execution from owning that GSD index, so the hook added a conflicting
instruction at each slice wrap-up.

### Occurrences

- Execution: SEED-009 story 28 first delivery / quick/127-consistent-web-trash / 42957ec3df
  - Timestamp: 2026-09-15T15:52:00+08:00
  - Tool: Cursor
  - Model: Cursor Grok 4.6
  - Open Dough release: 0.3.20
  - Evidence: after each PLAN.md status/proof edit in
    `.worktrees/127-consistent-web-trash/.planning/quick/127-consistent-web-trash/PLAN.md`,
    the session received `gsd- .planning/ write detected — ensure STATE.md
    reflects the latest phase and progress`. Coexistence guidance in
    `.cursor/rules/gsd-coexistence.mdc` says Open Dough execution does not
    update that index. STATE.md was left unchanged; all eleven slice wrap-ups
    still received the reminder.
  - Observed effect: extra hook context and a conflicting instruction on every
    plan update; no STATE.md edit was made.
  - Inference: a GSD progress hook keyed on any `.planning/` write cannot
    distinguish Open Dough PLAN maintenance from GSD phase progress; until the
    overlay is scoped, coordinators must ignore it rather than follow it.

## DD-053 — Eleven-slice sequential execution overflowed coordinator context mid-delivery

One planned Story Branch Mode run with eleven sequential slices, each with a
fresh implementation agent plus a fresh refactor agent and coordinator wrap-up,
exhausted the coordinator conversation before slice 8 delivery finished.
Resume from the retained plan identity and conversation summary recovered the
uncommitted collision test and completed the remaining slices.

### Occurrences

- Execution: SEED-009 story 28 first delivery / quick/127-consistent-web-trash / 42957ec3df
  - Timestamp: 2026-09-15T15:25:00+08:00
  - Tool: Cursor
  - Model: Cursor Grok 4.6
  - Open Dough release: 0.3.20
  - Evidence: continuation summary recorded slices 1–7 pushed, slice 8
    collision test uncommitted, refactor complete, PLAN not yet updated for
    slice 8; HEAD before resume was `91be697ee2`. After resume, slice 8 was
    formatted/committed as `9e471fe799` and slices 9–11 followed to
    `15e246cc7b`.
  - Observed effect: one context reset during wrap-up; no lost product work
    because the execution worktree, PLAN identity, and uncommitted test were
    intact.
  - Inference: a long sequential plan can overflow one coordinator session
    even when identity is retained; resume from the plan plus worktree is
    sufficient when wrap-up state is also kept in Git or the working tree.
