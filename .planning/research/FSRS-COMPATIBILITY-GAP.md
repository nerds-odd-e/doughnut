...
**Status:** First-rating initials on New (all four G, including Tutor **2** as Hard) are **closed** in product. **New** = ungraded is locked in ADR 0001. Remaining work in [`.planning/quick/011-new-ungraded-first-rating/PLAN.md`](../quick/011-new-ungraded-first-rating/PLAN.md) is Again-row backfill of still-New `AGAIN` / `AGAIN_ZERO` rows (not shipped). After that: **deferred** knobs (**E3** / **E4**) plus **accept ADR 0003** (human). Other shipped FSRS-6 locks, including same-hour success short-term next Stability (elapsed 0, S > 0), live in ADR 0003 Decision and code.
