...
- Slice 13: production path was already correct — `AuthoredNoteReferenceInboundFacade` re-resolves every candidate live and keeps only `Resolved` results, so a candidate that becomes `Ambiguous` (e.g. via a newly created same-titled namesake in a different folder of the same notebook — same-folder namesakes are blocked by `note.uk_note_notebook_folder_title`) is already excluded from both would-be targets' inbound lists; no production change needed, only a new regression test (`NoteControllerShowTests.namesakeCreationRemovesNowAmbiguousInboundReferenceFromBothTargets`). A viewer-visibility-under-ambiguity variant was judged already covered by `AuthoredNoteReferenceInboundFacadeTest.excludesAReferenceThatIsUnreadableToTheViewer` + `WikiLinkResolverReferenceResolutionTest.resolveReference_returnsAmbiguous_whenWikiPortablePathTargetHasSeveralReadableMatches` together (ambiguity is computed strictly over the viewer's readable candidate set), so none was added.
- Slice 14: `TextContentController.assertReferencedTitleRenameIsUnambiguous` (the inbound guard deciding whether a rename needs a reference-handling choice) now calls `authoredNoteReferenceInboundFacade.distinctReferrerNotesForViewer(...).isEmpty()` instead of the old cache-row-existence check, which made `ResolvedWikiLinkService.hasInboundResolvedWikiLinkRowsFromNonDeletedReferrers`/`InboundResolvedWikiLinks.hasRowsFromNonDeletedReferrers` dead and removed in refactor (`InboundResolvedWikiLinks` itself survives for `sampledReferencesNotesForFocusContext`, used elsewhere). `WikiLinkRewriteService.rewriteInboundWikiLinksForTitleRename` now computes a live-resolved referrer-id set via the facade *before* the title mutation (old link text can't resolve post-rename) and passes it to `WikiLinkRewriteSupport.applyInboundReferrerRewrite(..., Optional<Set<Integer>> liveResolvedReferrerIds)` to filter which cache-row-discovered referrers are actually rewritten — the other five non-title-rename callers of that method (location-change, notebook-move, folder-rename; Slice 16's territory) pass `Optional.empty()` and are otherwise untouched. Title-rename-specific orchestration was extracted to a new `TitleRenameWikiLinkRewrite` (80 lines) during refactor to keep `WikiLinkRewriteService`/`WikiLinkRewriteSupport` under the 250-line limit after this slice's additions; `WikiLinkRewriteService.rewriteInboundWikiLinksForTitleRename`'s public signature is unchanged. Tests in `TextContentControllerUpdateNoteTitleInboundWikiReferencesTests` now assert persisted `authored_note_reference` rows (via `AuthoredNoteReferenceRowTestSupport.rowsFor`) plus live facade resolution, replacing the old `ResolvedWikiLinkRepository`/`row.getDestinationNote()` assertions. Note-ID URL rewrite behavior on rename is unaffected (was already skipped by this rewrite path).
- Slice 15: `NoteReferenceHandling.removeNoteLinksFromReferrerProperties` now filters its referrer discovery by a live-resolved id set from `AuthoredNoteReferenceInboundFacade` (which gained a new public `distinctReferrerIdsForViewer(Note, User)`, extracted during refactor as a shared idiom with `TitleRenameWikiLinkRewrite`'s equivalent, which now delegates to it instead of its own deleted private helper) before still using `resolvedWikiLinkRepository.findRowsReferringToNonDeletedNotesForTarget` for the per-referrer authored link *text* (the facade only exposes `Note`/ids, never persistence rows or link text, by design — kept that boundary rather than widening it). The deleting user is the existing `viewer` already threaded through `NoteController.deleteNote` → `NoteService.destroy` → this method; no new viewer concept was introduced. A referrer whose reference has become ambiguous since it was cached is now correctly excluded from the property rewrite (new test `NoteControllerDeleteReferenceHandlingTests.shouldNotRemovePropertyReferenceThatHasBecomeAmbiguousSinceItWasCached`); `NoteControllerDeleteTests` was split during refactor into that new class (delete-time reference-handling policy tests) plus a slimmed `NoteControllerDeleteTests` (soft-delete/memory-tracker concept only) to stay under the file-size limit, mirroring the repo's existing sibling `NoteControllerDelete*Tests` convention. Content-writing mechanics and memory-tracker/orphan-image cleanup were untouched.
...
- Slice 16: `AuthoredNoteReferenceInboundFacade` gained `distinctInboundReferencesForViewer(Note, User)` returning `InboundReference(Note referrer, List<String> authoredLinkTexts)` — a generalization of `distinctReferrerNotesForViewer` that also carries each surviving candidate's exact authored link text (via `AuthoredNoteReferenceRow.toDomainReference().authoredLink()`), reusing the same candidate/resolve/visibility helpers; `distinctReferrerNotesForViewer` now derives from it instead of duplicating the loop. This closed the last gap in relocation/rewrite candidate discovery: `WikiLinkRewriteSupport.applyInboundReferrerRewrite` previously still queried `ResolvedWikiLinkRepository.findRowsReferringToNonDeletedNotesForTarget` for `(referrer, linkText)` pairs and only used the facade to filter that result down to currently-live-resolved ids (the pattern Slice 14/15 established); it now takes the facade-captured `Map<Integer, List<String>>` directly, so the `Optional<Set<Integer>>` intersection-filter mechanism from Slice 14 is gone entirely, and `captureLiveResolvedReferrerIds*` were renamed/generalized to `captureLiveResolvedInboundReferences*` (now on the new `WikiLinkReferenceCapture` collaborator) to reflect that they capture link text, not just ids. Outgoing co-moved-target rewrite (`coMovedTargetsByAuthoredLink`, cross-notebook folder move) similarly stopped reading `ResolvedWikiLinkRepository.findBySourceNote_IdOrderByIdAsc`: it now parses the moved note's own live content via `AuthoredNoteReferences` and resolves each reference live — captured *before* the move (`captureLiveResolvedOutgoingWikiLinksToCoMovedNotes`), because resolution must happen against the pre-move notebook scope to correctly detect a same-titled collision already in the destination notebook. `ResolvedWikiLinkRepository` is now fully removed from `WikiLinkRewriteService`, `WikiLinkRewriteSupport`, `FolderRelocationService`, `RelationController`, and `TitleRenameWikiLinkRewrite`; `resolvedWikiLinkService.refreshCardinalityAcrossMovedNotebooks` (a different class) is untouched and out of scope. Six controller tests' referrer fixtures had to move off the builder's raw `.content(text)` (which bypasses `authored_note_reference` population) onto a shared `MakeMe.authorReferencingContent(Note, String)` helper, since the new candidate discovery reads that table — a fixture-population fix, not a behavior change. Post-change-refactor split both `FolderRelocationService` (279→156 lines, extracting `FolderMoveRelocation`) and `WikiLinkRewriteService` (326→191 lines, extracting `WikiLinkReferenceCapture` and `WikiLinkRelocationRewrite`) to stay under the file-size limit; `WikiLinkRewriteService`'s public signatures are unchanged so no other call sites moved.
