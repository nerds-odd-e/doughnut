...
- **Learning:** two shared e2e helpers not itemized above needed mechanical updates because they used `[data-test="assimilate"]` as a generic "panel ready/absent" signal, which can now legitimately be absent (a mode may render a status link instead of a button): `e2e_test/start/pageObjects/noteMoreOptionsForm.ts`'s `openAssimilationSettings()` and `noteConversationAndQuestionMethods.ts`'s `expectWithoutAssimilationPanel()` — both now key off the always-present `[data-testid="assimilation-modes"]` container instead. Verified this didn't regress the 4 slice-7 out-of-scope features (re-ran all 4: 40/40 still green — the old table is still mounted, so their tracker-existence/recall-count assertions are untouched). Post-change-refactor also found and removed dead code this cutover created: the note-level `assimilateDisabled` prop threading (`AssimilationSettings.vue`, `AssimilationPanel.vue`) and `useMemoryTrackerActions.ts`'s now-unused `assimilateDisabled` computed, plus `hasNoteLevelTrackerOfType`/`hasUnderstandingNoteLevelTracker` in `assimilationMemoryTrackers.ts` (last callers were the deleted `showCommissionedOption`/`showSpellingOption` gating and the removed `assimilateDisabled` computed) — `revive`/`removeFromRecall` handlers themselves stay live via the property-level panel until slice 8.
