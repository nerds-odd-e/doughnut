...
allowed. All no-recreation promises above map to this leaf.
...
Check retirement state while holding the acquired gate before admitting a
runner; clean up only the gate acquired by this invocation on refusal. Keep
retirement's explicit retry permission. Use existing wrapper fixtures and a
test-side process barrier; no timing sleeps as the correctness oracle and no
new product scheduling hooks. Remove superseded marker-check test duplication
and stale “later retirement” comments in the implicated admission module.

Focused verification: `CURSOR_DEV=true nix develop -c node --test
scripts/backend-test-worktree-admission.test.mjs
scripts/worktree-retirement-admission.test.mjs
scripts/sut-retirement-admission.test.mjs`.

Sizing: ~5–8 minutes, medium confidence; one admission decision and one
controlled interleaving proof. Scrutinize after five minutes; at ten minutes
split/refine if fixture work hides another outcome. No new ownership layer.
