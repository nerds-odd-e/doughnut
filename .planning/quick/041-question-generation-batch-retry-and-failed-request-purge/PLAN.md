...
**Status:** done — both slices shipped; production purge gate enabled by developer instruction  
...
- **Production enablement:** developer explicitly directed enabling the gate (not left to agent judgment). `application-prod.yml`'s `question_generation_batch_failed_request_purge` is now `1=1` directly, no env-var toggle — unlike the sibling `question_generation_batch_incomplete_purge` placeholder, this one is left permanently on rather than reverted after apply, since Flyway versioned migrations run at most once per database (tracked in `flyway_schema_history`); the placeholder value is irrelevant to that database once `V300000318` has applied. Docs one-time-purge note and removing the migration-only test are left for a follow-up once the deploy has actually run against production (not done yet as part of this config change).
