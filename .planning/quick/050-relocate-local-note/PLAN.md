...
(`source /workspace/scripts/cloud_agent_setup.sh && pnpm backend:test_only`).
Canonical shape in
`NotebookGitProposalRelocationControllerTest.relocatesANoteIntoANestedReadmeOnlyFolderDistinguishedByFullPath`
(original ID, Courses/Physics ancestors, unchanged bytes, exact head/tree).
Siblings assert only destination: root→Folder, Folder→root, Source→Dest.
Combined parent+filename still rejected by
`rejectsACombinedParentAndFilenameChangeWithoutMutatingTheAcceptedBinding`.
...
Backend suite (`source /workspace/scripts/cloud_agent_setup.sh && pnpm backend:test_only`).
`NotebookGitProposalRelocationPrivateAssociationControllerTest.preservesPrivateAssociationsWhenRelocatingAcrossParentsAndLeavesTheIdenticalTextNoteUntouched`
observes relocated tracker (removedFromTracking, nextRecallAt), MCQ, and
conversation still on the original note ID; untouched identical-content note
keeps its own tracker. Canonical same-parent shape remains in
