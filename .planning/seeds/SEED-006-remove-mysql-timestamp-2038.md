...
status: dormant
planted: 2026-08-18
planted_during: quick/013-fsrs-maximum-interval
...
Quick plan 013 converts only `memory_tracker.last_recalled_at` and `next_recall_at`. Other `TIMESTAMP` columns (including `assimilated_at`, created/updated/deleted, job locks) still hit 2038.
...
- `.planning/quick/013-fsrs-maximum-interval/PLAN.md` — DATETIME for the two recall-due columns only
