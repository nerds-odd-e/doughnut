...
- For an already Git-backed notebook whose web tree matches its accepted Git
  tree, each successful content save that changes the Portable tree appends a
  commit with the previous accepted tip as its parent. Existing commit IDs and
  ancestry remain unchanged, including saves close together on the same note
  before any client downloads the tip.
- Replace the existing same-note, unexposed-tip amendment behavior for these
  saves. There is no new delayed batch, timer, or sealing workflow: each
  successful changed save establishes its own accepted revision. A save that
  leaves the Portable tree unchanged still creates no commit.
- Preserve existing content/frontmatter handling, authorization, note identity,
  learning data, and save atomicity. Preserve existing behavior for notebooks
  without Git and for a web tree already differing from accepted Git; bringing
  such a tree back into sync is not this story's prerequisite repair work.
- **Deferred promises:** Deletion, Trash, Undo, Restore, recovery through Move,
  and Git/local trash compatibility; note creation, rename or move; expanded
  support for Readmes or other concept types; accumulated local publication,
  divergence reconciliation, performance targets, and history UI. Existing
  supported operations remain supported. These are delivery boundaries, not
  new rejection rules or reasons to introduce separate persistence mechanisms.
- Do not rewrite old history or reconstruct commits already replaced before
  this change. Existing amendment eligibility must not allow a subsequent
  content save to replace the current tip after the change takes effect.

#### Key examples

1. **Rapid saves before download:** Accepted tip is `A`. The owner changes
   `Biology/Cells` content and saves, producing `B`, then changes it again within
   the former amendment window and saves, producing `C`. No client downloaded
   `B`; history is still `A → B → C`, with `B` unchanged and its content
   recoverable from Git.
2. **Pull after several saves:** A clean local checkout is at `A`; the web has
   saved `B` and `C`. Existing pull fast-forwards to `C` and retains both
   intermediate revisions. Downloading `B` before the second save also leaves
   the same append-only ancestry; exposure does not determine commit policy.
3. **Unchanged save:** With accepted tip `C`, the owner saves content that
   leaves the Portable tree unchanged. The accepted tip remains `C`.
4. **Existing amendment candidate:** The current tip was recorded as eligible
   for amendment under the old behavior. The next changed content save after
   this change appends a child of that tip and preserves it.

- **Existing-behavior evidence:** `WebNoteContentSaveService` already skips Git
  writes for unchanged trees and pre-existing web/Git mismatch.
  `AcceptedSnapshotPersistence.persistOrdinaryNoteContentEdit` currently
  permits same-note tip replacement within ten minutes. This identifies the
  behavior to change; it is not an implementation plan or executed proof.
- **Decision context:** Follow [ADR 0004 — OKF-compatible notebook Markdown
  profile](../../docs/adrs/0004-okf-compatible-notebook-markdown-accepted.md)
  for authored content and portable paths. Trash remains location-based under
  that record, with no lifecycle change here. ADR 0002 remains Proposed; its
  batching model is not a binding prerequisite. The owner's append-only
  direction replaces the prior amendment contract for this selected workflow.
- **Effort hypothesis:** S–M (30 minutes–2 hours), medium confidence; existing
  save and pull paths are reused, with amendment expectations to reconcile.
- **Depends on / safe stopping point:** Existing Git-backed content saves,
  clone and clean fast-forward pull. No dependency on trash navigation (story
  34) or another new story; this content-editing workflow remains useful if
  later stories are deferred. Parallel execution should use separate worktrees
  and coordinate any shared seed edits or newly discovered shared code changes.
- **Open decisions:** None blocking this narrow refinement.
