...
refused for folders with files. The
...
- **Value:** the refusal loses nothing and a local workaround exists (copy the
  files between two checkouts), so this is a convenience.
...
in other notebooks reaching Git are delivered; the rarer cross-notebook move of
files (story 10, a convenience whose absence loses nothing) remains.
