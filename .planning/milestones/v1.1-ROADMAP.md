...
During spelling recall, an answer that names a different note becomes a learning opportunity (accidental match → light penalty, reveal both, offer link). Overlapping-but-distinct notes stay distinct via alias-as-wiki-link declaration and try-again with no SRS credit.
...
**Goal:** The backend→frontend answer contract can represent special outcomes (`ACCIDENTAL_MATCH` with matched-note id, and `OVERLAP`) beyond the boolean `correct` flag.
**Mode:** mvp
...
**Success Criteria** (what must be TRUE):
  ...
  2. The `AnsweredQuestion` response carries matched-note topology; special grades use `Answer.outcome` (`ACCIDENTAL_MATCH` / `OVERLAP`).
...
- [x] 01-01-PLAN.md — Extend the Answer/AnsweredQuestion contract (`@Transient matchedNoteId` + `AnswerOutcome` on Answer; optional `matchedNotes` on AnsweredQuestion), regenerate the OpenAPI client, verify the @Transient fields surface in types.gen.ts (A1), confirm the frontend type-checks, and pin representable-but-not-returned with no-behavior backend tests on both contract surfaces (AnsweredQuestion + RecallPromptHistoryItem)
...
**Mode:** mvp
**Depends on**: Phase 1 (`Answer.outcome` in the contract); independent of Phases 2–4
...
- [x] 06-03-PLAN.md — Edge cases: dead/unreadable/self → ordinary correct (`correct: true`, no special outcome); AM + matched-notes gates
...
- Accidental-match penalty: third SRS outcome via `partialFail` (−10, no 12h)
- Overlap declared via wiki-link values in `aliases` (not auto-detected)
- Link building is user-initiated only (MatchedNoteLinkOffer)
- OVERLAP persists `quiz_answer.outcome` and is excluded from wrong-count

**Issues Deferred:**
- MCQ accidental-match, fuzzy matching, Notebook:Title typing (v2)
- Formal `/gsd-audit-milestone` skipped by acknowledgment at close

**Technical Debt:**
- None recorded for this milestone beyond deferred v2 scope
