...
1. **Graph-structural** — where the note sits (Doughnut's current approach: BFS over wiki links).
...
The key insight for Doughnut: **in a zettelkasten, wiki links ARE concept anchors.** Each linked note is an atomic concept node with a descriptive title — that's the zettelkasten convention. So Savaal's "extract concepts → retrieve per concept" *collapses* in Doughnut: the focus note's linked notes already *are* the concept groups.

**Doughnut's BFS-over-wiki-links retrieval is already doing implicit concept-grouped retrieval.** The question was never "should we switch to concept-grouped" — we're already there. The real design choices are two different ones (see the companion note on summarization, and SEED-007 on prompt splitting).
