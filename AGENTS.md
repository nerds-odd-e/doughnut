...
Repository instructions for Codex and other AI coding agents.
...
`.agents/skills/problem-decomposition/SKILL.md`; planning artifacts and lifecycle:
...
Portable digest (details live in the cited always-on skills — keep `AGENTS.md` and `CLAUDE.md` in sync):

1. High cohesion — one concept, one place (`general` skill)
2. Keep it simple — minimum code; no defensive programming (`general` skill)
3. Capability naming — no planning sequence numbers in product artifacts (`general` and `planning` skills)
...
- **Story refinement:** use `.agents/skills/dough-story-refinement/SKILL.md` to clarify selected stories' goal, scope, and key examples in their home seeds before slice planning. Apply conservative scope and post-implementation cleanup from the `planning` skill.
