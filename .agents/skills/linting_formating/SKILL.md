...
- **Biome**: Frontend, CLI, MCP server, `packages/donut-test-fixtures`, Cypress, and other root-level TypeScript/JavaScript code
- **Redocly CLI**: OpenAPI specification validation
- **Backend linting**: Java code quality checks (via Gradle)

## Linting Philosophy

**IMPORTANT**: Linting is primarily for CI/CD validation. During development,
format changed components explicitly before staging; the pre-commit hook only
validates staged components and never formats or stages files.
...
- Shared API test fixtures (`packages/donut-test-fixtures`) via Biome (`pnpm test-fixtures:lint`)
...
- `pnpm test-fixtures:lint` - Biome on `packages/donut-test-fixtures` only
- `pnpm cy:lint` - Cypress E2E test linting
- `pnpm openapi:lint` - OpenAPI spec validation

## Format vs Lint

- **Use `./scripts/run.sh pnpm format:changed`** before staging — it selects
  affected components from staged, unstaged, and nonignored untracked paths,
  then applies their formatter commands. During slice wrap-up, the coordinator
  runs it once after refactor/API generation and before the PLAN update; it does
  not pre-filter paths or spawn a formatting agent. Repeat only after an
  intervening repair invalidates that preparation. The **format-changed** skill
  remains available for explicit on-demand use.
- Use `CURSOR_DEV=true nix develop -c pnpm format:all` only when intentionally
  formatting the entire repository.
- The pre-commit hook runs `pnpm lint:changed` against affected staged
...
This project uses `@biomejs/biome@2.4.x` across the workspace (`package.json`, `frontend/package.json`, `packages/donut-test-fixtures/package.json`). Keep those versions pinned and in sync — `syncpack fix` runs on `postinstall` to help.

### Monorepo config layout

- **`biome.json`** (workspace root) — governs `e2e_test/`, `cli/`, `mcp-server/`, `scripts/`, `infra/`, `packages/donut-test-fixtures/`, etc. Implicit `root: true`.
