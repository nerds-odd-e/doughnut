...
2. If it would, compare the named removal target with the explicitly named
   ...
   the source's promises and dependencies instead. Keep the comparison
...
4. In planned execution, if the current instruction is unambiguously stale
   ...
   before delegation.
5. If a quick source conflicts with the removal, or if both planned outcomes
   ...
   outcomes and the decision required; do not rewrite the source or create a plan
   from the quick path.
